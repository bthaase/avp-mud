...
void do_arm( CHAR_DATA* ch, char* argument )
{
    ...
    if ( IS_NPC( ch ) )
    {
        if ( ch->pIndexData )
        {
            sprintf( name, "%d", ch->pIndexData->vnum );
        }
    }
    else
    {
...
void do_reload( CHAR_DATA* ch, char* argument )
{
    ...
    if ( !str_cmp( arg1, "main" ) )
    {
        ...
        if ( obj )
            reload_main( ch, obj, arg2 );

        if ( obj2 )
            reload_main( ch, obj2, arg2 );

        return;
    }

    if ( reload_sentry( ch, arg1, arg2 ) )
        return;
...
void do_switch( CHAR_DATA* ch, char* argument )
{
    ...
    if ( ch->vision == RACE_MARINE )
        useMode[0] = TRUE;
    else if ( ch->vision == RACE_ALIEN )
        useMode[1] = TRUE;
    else if ( ch->vision == RACE_PREDATOR )
        useMode[2] = TRUE;
    else
        useMode[3] = TRUE;
...
int use_obj( CHAR_DATA* ch, char* argument, int mode )
{
    ...
    */
    ...
    else if ( device->item_type == ITEM_TOOLCHEST )
    {
        ...
        {
            if ( work->item_type != ITEM_ARMOR )
                continue;

            if ( work->wear_loc < 0 || work->wear_loc > MAX_WEAR )
                continue;
    ...
    else if ( device->item_type == ITEM_AMMOBOX )
    {
        ...
        for ( work = ch->first_carrying; work; work = work->next_content )
        {
            ...
            if ( work->ammo )
            {
                if ( ( pAmmo = get_obj_index( work->ammo->pIndexData->vnum ) ) == NULL )
                    continue;
            ...
            if ( work->attach )
            {
                if ( work->attach->ammo == NULL )
                    continue;

                if ( ( pAmmo = get_obj_index( work->attach->ammo->pIndexData->vnum ) ) == NULL )
                    continue;
    ...
    else if ( device->item_type == ITEM_SPAWNER )
    {
        ...
        if ( device->weight - weight < 0 )
        {
            weight = device->weight - 1;
        }
        ...
        if ( mode == 2 )
        {
            ...
            else
            {
                if ( rem_sentry( device, ch ) == TRUE )
                {
                    ...
                    if ( device->in_room )
                    {
                        if ( ch->carry_number + get_obj_number( device ) > can_carry_n( ch ) )
                            return 0;

                        if ( ch->carry_weight + get_obj_weight( device ) > can_carry_w( ch ) )
                            return 0;
...
void do_eat( CHAR_DATA* ch, char* argument )
{
    ...
    switch ( obj->item_type )
    {
        case ITEM_FOOD:
            ...
            if ( !IS_NPC( ch ) )
                send_to_char( "&CYou feel very refreshed.\n\r", ch );
...
int allowedmp( DESCRIPTOR_DATA* d )
{
    ...
    for ( mp = mplist; mp; mp = mp->next )
        if ( ( ( strstr( mp->host, d->host ) ) ) || ( !( str_cmp( mp->name, d->character->name ) ) ) )
            return 1;
    ...
    for ( tmp = first_sentry; tmp; tmp = tmp->next )
    {
        ...
        for ( dir = 0; dir <= 9; dir++ )
        {
            ...
            switch ( tmp->arc )
            {
                ...
                case DIR_NORTHWEST:
                    if ( dir != DIR_NORTH && dir != DIR_WEST && dir != DIR_NORTHWEST )
                        continue;

                    break;

                case DIR_NORTHEAST:
                    if ( dir != DIR_NORTH && dir != DIR_NORTHEAST && dir != DIR_EAST )
                        continue;

                    break;

                case DIR_SOUTHWEST:
                    if ( dir != DIR_SOUTH && dir != DIR_SOUTHWEST && dir != DIR_WEST )
                        continue;

                    break;

                case DIR_SOUTHEAST:
                    if ( dir != DIR_SOUTH && dir != DIR_SOUTHEAST && dir != DIR_EAST )
                        continue;
            ...
            if ( ( pexit = get_exit( gun->in_room, dir ) ) == NULL )
                continue;
            ...
            for ( dist = 1; dist <= gun->value[2]; dist++ )
            {
                ...
                if ( pexit->distance > 1 )
                    to_room = generate_exit( gun->in_room, &pexit );
                ...
                if ( ( pexit = get_exit( my_room, dir ) ) == NULL )
                    break;
            }
        }
...
void deploy_fire( SENTRY_DATA* gun, int dir )
{
    ...
    for ( range = 1; range <= mrange; range++ )
    {
        ...
        if ( pexit->distance > 1 )
            to_room = generate_exit( weap->in_room, &pexit );
