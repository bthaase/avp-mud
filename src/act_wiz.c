...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
    ...
    for ( level = slevel; level > 100 ; level-- )
    {
        ...
        if ( level > 105 && level < 200 ) level = 105;
...
void echo_to_room( sh_int AT_COLOR, ROOM_INDEX_DATA *room, char *argument )
{
    ...
    {
        if ( AT_COLOR > -1 ) set_char_color( AT_COLOR, vic );
...
void do_snoop( CHAR_DATA *ch, char *argument )
{
	...
	for ( d = first_descriptor; d; d = d->next )
        {
            for ( i = 0; i <= 5; i++ )
            {
                if ( d->snoop_by[i] == ch->desc ) d->snoop_by[i] = NULL;
            }
        }
        return;
    }
    ...
    for ( i = 0; i <= 5; i++ )
    {
        if ( victim->desc->snoop_by[i] != NULL ) cnt++;
    }
...
*/
    ...
    for ( i = 0; i < amount; i++ )
    {
        ...
        else
        {
            ...
            if ( i == 1 ) act( AT_IMMORT, "$n has created $p!", ch, obj, NULL, TO_ROOM );
        }
    }
...
{
    if ( !str_cmp( argument, "on" ) ) sysdata.ALLOW_OOC = TRUE;
    else if ( !str_cmp( argument, "off" ) ) sysdata.ALLOW_OOC = FALSE;
    else sysdata.ALLOW_OOC = !sysdata.ALLOW_OOC;
  ...
  for ( icnt = 0; icnt < MAX_KEY_HASH; icnt++ )
  {
    for ( rid = room_index_hash[icnt]; rid; rid = rid_next )
    {
      ...
      for ( exit = rid->first_exit; exit; exit = exit_next )
      {
        ...
        tf = TRUE;
        if ( !exit ) tf = FALSE;
        else if ( !exit->to_room ) tf = FALSE;
        else if ( !exit->to_room->area ) tf = FALSE;
        else if ( exit->to_room->area == pArea ) tf = TRUE;
        else if ( rid->area == pArea ) tf = TRUE;
