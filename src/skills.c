...
void do_scan( CHAR_DATA *ch, char *argument )
{
  ...
  if ( get_curr_per( ch ) >= 10 ) max_dist++;
  if ( get_curr_per( ch ) >= 20 ) max_dist++;
  if ( get_curr_per( ch ) >= 30 ) max_dist++;

  if ( ch->race == RACE_ALIEN ) max_dist += 2;
  if ( ch->race == RACE_PREDATOR && ch->vision > -1 ) max_dist += 2;
  if ( ch->race == RACE_PREDATOR && ch->vision < 0 ) max_dist += 1;
  ...
  for ( dist = 1; dist <= max_dist; )
  {
    ...
    if ( can_see )
    {
       ...
       send_to_char( "\n\r", ch );
       if ( !is_spectator(ch) ) run_awareness( to_room->first_person, ch );
...
void do_treat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( ch ) ) levelA = ch->pcdata->learned[gsn_advanced_medical];
    if ( !IS_NPC( ch ) ) levelB = ch->pcdata->learned[gsn_basic_medical];
    ...
    if ( levelA > 0 )
    {
       ...
       if ( !IS_NPC( ch ) ) ch->pcdata->prepared[gsn_advanced_medical] = 0;
    ...
    else if ( levelB > 0 )
    {
       ...
       if ( !IS_NPC( ch ) ) ch->pcdata->prepared[gsn_basic_medical] = 0;
...
void do_spit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( ch ) ) level = ch->pcdata->learned[gsn_acidspit];
    ...
    if ( level > 0 )
    {
       ...
       else
       {
          ...
          if ( rtn != rOBJ_SCRAPPED ) rtn = damage_obj( obj, level + 2 );
          if ( rtn != rOBJ_SCRAPPED ) rtn = damage_obj( obj, level + 2 );
          if ( rtn != rOBJ_SCRAPPED ) rtn = damage_obj( obj, level + 2 );
...
void do_request( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( ch ) ) level = ch->pcdata->learned[gsn_requestassist];
    ...
    if ( ( pMobIndex = get_mob_index( MOB_VNUM_TEMPLATE ) ) == NULL ) return;         
    ...
    if ( !str_cmp( arg, "sniper" ) && level >= 1 )
    {
       ...
       if ( !IS_NPC( ch ) ) ch->pcdata->prepared[gsn_requestassist] = 0;
       ...
       for ( i = 0; i < 2; i++ )
       {
           ...
           if ( i == 0 ) pObjIndex = get_obj_index( 681 );  // Scope Ammo
           if ( i == 1 ) pObjIndex = get_obj_index( 686 );  // Slugshot
           ...
           if ( i == 0 ) pObjIndex = get_obj_index( 680 );  // Scope Rifle
           if ( i == 1 ) pObjIndex = get_obj_index( 684 );  // Shotgun
    ...
    else if ( !str_cmp( arg, "assault" ) && level >= 2 )
    {
       ...
       for ( i = 0; i < 3; i++ )
       {
           ...
           if ( i == 2 ) pObjIndex = get_obj_index( 674 );  // M56 Ammo
           if ( i != 2 ) pObjIndex = get_obj_index( 81 );   // M309 Ammo
           ...
           if ( i == 2 ) pObjIndex = get_obj_index( 673 );  // Smartgun
           if ( i != 2 ) pObjIndex = get_obj_index( 80 );   // Pulse Rifle
    ...
    else if ( !str_cmp( arg, "support" ) && level >= 3 )
    {
       ...
       for ( i = 0; i < 3; i++ )
       {
           ...
           if ( i == 0 ) pObjIndex = get_obj_index( 163 );  // M90 Ammo
           if ( i == 1 ) pObjIndex = get_obj_index( 674 );  // M56 Ammo
           if ( i == 2 ) pObjIndex = get_obj_index( 81 );   // M309 Ammo
           ...
           if ( i == 0 ) pObjIndex = get_obj_index( 162 );  // Minigun
           if ( i == 1 ) pObjIndex = get_obj_index( 673 );  // Smartgun
           if ( i == 2 ) pObjIndex = get_obj_index( 80 );   // Pulse Rifle
...
void do_confuse( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( ch ) ) level = ch->pcdata->learned[gsn_confuse];
...
void do_rage( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->pcdata ) level = ch->pcdata->learned[gsn_alien_rage];
...
void do_hive ( CHAR_DATA *ch , char *argument )
{
    ...
    if ( is_spectator( ch ) ) { send_to_char( "&RSpectator mode. Please wait for respawn.\n\r", ch ); return; }
    if ( ch->vent ) { send_to_char( "&RYou can't hive a room while your inside a vent.\n\r", ch ); return; }
    ...
    if ( xIS_SET( ch->in_room->room_flags, ROOM_HIVED ) ) return;

    hflag_toggle( ch->in_room );
    xTOGGLE_BIT( ch->in_room->room_flags, ROOM_HIVED );

    if ( curr_arena ) curr_arena->hive++;
...
void do_dehive ( CHAR_DATA *ch , char *argument )
{
    ...
    { 
    	default:
                ...
                // Predator Penalty
                if ( ch->race == RACE_PREDATOR ) ticks += 2;

                // Trapped Penalty
                if ( count_traps(ch->in_room) > 0 ) ticks += 2;
...
void do_lunge( CHAR_DATA * ch, char * argument )
{
    ...
    if ( !IS_NPC( ch ) )
    {
        if ( ch->pcdata->learned[gsn_lunge] == 1 )
         { mrange = 2; stun = 3; multistun = FALSE; }
        if ( ch->pcdata->learned[gsn_lunge] == 2 )
         { mrange = 3; stun = 6; multistun = FALSE; }
        if ( ch->pcdata->learned[gsn_lunge] == 3 )
         { mrange = 3; stun = 6; multistun = TRUE; }
    ...
    for ( range = 1; range <= mrange; range++ )
    {
       ...
       if ( ( rch = get_char_room( ch, arg2 ) ) != NULL ) found = TRUE;
       ...
       if ( found )
       {
           ...
           {
               if ( rch->race == RACE_PREDATOR ) stun = 5;
               ...
               if ( to_room->area ) motion_ping( to_room->x, to_room->y, to_room->z, to_room->area, ch );
               return;
           }
       }
       ...
       {   
           ...
           for ( rch = to_room->first_person; rch; rch = rtmp )
           {
              ...
              if ( rch->race == RACE_PREDATOR ) tstun = 4;
...
bool destroy_door( ROOM_INDEX_DATA * room, EXIT_DATA * pexit, bool bonus )
{
    ...
    if ( room == NULL || pexit == NULL ) return FALSE;

    if ( !xIS_SET( pexit->exit_info, EX_CLOSED ) ) return FALSE;
    ...
    {
       if ( ( rexit = get_exit( pexit->to_room, rev_dir[pexit->vdir] ) ) == NULL ) return val;

       if ( destroy_door( pexit->to_room, pexit->to_room, FALSE ) ) val = TRUE;
    }
...
void do_leap( CHAR_DATA * ch, char * argument )
{
    ...
    if ( is_spectator( ch ) ) { send_to_char( "&RYou may not activate skills while in spectator mode.\n\r", ch ); return; }
    ...
    if ( ch->pcdata->learned[gsn_leap] == 3 ) antidoor = TRUE;

    if ( arg2[0] != '\0' && atoi(arg2) > 0 ) mrange = URANGE( 1, atoi(arg2), mrange );
    ...
    for ( range = 1; range <= mrange; range++ )
    {
       ...
       if ( xIS_SET( pexit->exit_info, EX_NOVENT ) && ch->vent == TRUE ) break;
...
void do_construct ( CHAR_DATA *ch , char *argument )
{
    ...
    if ( !curr_arena ) return;

    if ( is_spectator( ch ) ) { send_to_char( "&RSpectator mode. Please wait for respawn.\n\r", ch ); return; }
    if ( ch->vent ) { send_to_char( "&RYou can't construct while your inside a vent.\n\r", ch ); return; }
    if ( ch->race != RACE_ALIEN && get_trust(ch) <= 105 ) { do_nothing( ch, "" ); return; }
    ...
    if ( !str_cmp( arg, "cubby" ) )            { cost = 20; vnum = 297; }
    else if ( !str_cmp( arg, "acid trap" ) )   { cost = 10; vnum = 296; }
    else if ( !str_cmp( arg, "sentry" ) )      { cost = 10; vnum = 0; }
    else if ( !str_cmp( arg, "ambush" ) )      { cost = 40; vnum = 298; support = 1; }
    else if ( !str_cmp( arg, "entangle" ) )    { cost = 12; vnum = 294; }
    else if ( !str_cmp( arg, "tunnel" ) )      { cost = 40; vnum = 299; support = 1; }
    else if ( !str_cmp( arg, "barrier" ) )     { cost = 15; vnum = 0; }
    else if ( !str_cmp( arg, "resin pool" ) )  { cost = 50; vnum = 281; }
    else if ( !str_cmp( arg, "regenerate" ) )  { cost = 40; vnum = 282; }
    else if ( !str_cmp( arg, "acid burst" ) )  { cost = 20; vnum = 283; }
    else if ( !str_cmp( arg, "shock web" ) )   { cost = 25; vnum = 284; }
    else if ( !str_cmp( arg, "lure" ) )        { cost = 16; vnum = 295; }
...
void do_sc( CHAR_DATA * ch, char * argument )
{
    ...
    for ( range = 1; range <= mrange; range++ )
    {
       ...
       if ( ( rch = get_char_far_room( ch, to_room, arg2 ) ) != NULL ) found = TRUE;
    ...
    for ( range = 1; range <= mrange; range++ )
    {
       ...
       if ( found )
       {
           if ( !is_spectator( rch ) && cansee && can_see( ch, rch ) && !IN_VENT( rch ) )
           {
               ...
               if ( ch->vision == rch->race ) accuracy += 60;
