    ...
    else
    {
        ...
        for ( paf = ch->first_affect; paf; paf = paf->next )
        {
            ...
            if ( ( skill = get_skilltype( paf->type ) ) != NULL )
            {
                ...
                {
                    if ( paf->duration < 25 )
                        ch_printf( ch, "&z(&Y%5d&z )   ", paf->duration );
                    else if ( paf->duration < 6  )
                        ch_printf( ch, "&z(&R%5d&z )   ", paf->duration );
                    else
                        ch_printf( ch, "&z(&C%5d&z )   ", paf->duration );
                }
            ...
            if ( obj->wear_loc == iWear )
            {
                ...
                if ( can_see_obj( ch, obj ) )
                {
                    ...
                    switch ( obj->item_type )
                    {
                        ...
                        case ITEM_GRENADE:
                            if ( obj->value[4] > 0 )
                                ch_printf( ch, " &R[ARMED] &B(&C%d Rounds&B) ", obj->value[4] );

                            if ( obj->value[4] < 1 )
                                strcat( buf, " &B(&CUnarmed&B) " );
...
char* reduce_ratio( int a, int b )
{
    ...
    for ( x = a; x > 0; x-- )
    {
        ...
        if ( ( float )( ( int )( tmpA ) ) != ( float )( tmpA ) )
            continue;

        if ( ( float )( ( int )( tmpB ) ) != ( float )( tmpB ) )
            continue;
