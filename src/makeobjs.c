...
void make_corpse( CHAR_DATA* ch, CHAR_DATA* killer )
{
    ...
    for ( obj = ch->first_carrying; obj; obj = obj_next )
    {
        ...
        {
            if ( IS_OBJ_STAT( obj, ITEM_ALIEN ) && ch->race == RACE_ALIEN )
                continue;

            if ( IS_OBJ_STAT( obj, ITEM_MARINE ) && ch->race == RACE_MARINE )
                continue;

            if ( IS_OBJ_STAT( obj, ITEM_PREDATOR ) && ch->race == RACE_PREDATOR )
                continue;
        }
