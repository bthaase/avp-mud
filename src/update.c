...
int hit_gain( CHAR_DATA *ch )
{
	...
	switch ( ch->position )
	{
        ...
        case POS_MORTAL:   return -10;
        case POS_INCAP:    return 1;
        case POS_STUNNED:  return get_curr_rec(ch) * 2;
        case POS_PRONE:    gain += get_curr_rec(ch) * 2;  break;
        case POS_KNEELING: gain += get_curr_rec(ch) * 1;  break;
	}
    ...
    if ( ch->swarm ) gain = (int)( (float)(gain) * (float)(1.1) );    
...
int move_gain( CHAR_DATA *ch )
{
	...
	case POS_STUNNED:  return 1;
        case POS_PRONE:    gain *= 2;    break;
        case POS_KNEELING: gain *= 1;    break;
        }
    }
...
int field_gain( CHAR_DATA *ch )
{
	...
	{
	case POS_DEAD:	   return 0;
        case POS_MORTAL:   return -3;
        case POS_INCAP:    return -2;
        case POS_STUNNED:  return -1;
        }
    }
...
{   
    ...
    */
        ...
        {
           if ( IS_BOT( ch ) ) { match_log( "KILL;'BOT:%s' bled to death.", ch->name ); }
           else if ( IS_NPC( ch ) ) { match_log( "KILL;'NPC:%s' bled to death.", ch->name ); }
           else { match_log( "KILL;'PC:%s' bled to death.", ch->name ); }
	    ...
	    OBJ_DATA *obj;
            ...
            if ( xIS_SET( ch->affected_by, AFF_CLOAK ) )
            {
               if ( ( ch->field -= 4 ) <= 0 )
               {
                    ...
                    {
                        if ( obj->item_type == ITEM_CLOAK ) REMOVE_BIT( obj->value[0], BV00 );
                    }
               }
            }
	}
...
/* Corpse decay (npc corpses decay at 8 times the rate of pc corpses) - Narn */
	...
	/* if we get this far, object's timer has expired. */
	 ...
	 case ITEM_FIRE:
	  ...
	  if ( obj->carried_by )
	  { 
            if ( AT_TEMP >= 0 ) act( AT_TEMP, message, obj->carried_by, obj, NULL, TO_CHAR );
	  }
	  else if ( obj->in_room
	  &&      ( rch = obj->in_room->first_person ) != NULL
	  &&	!IS_OBJ_STAT( obj, ITEM_BURRIED ) )
	  {
            if ( AT_TEMP >= 0 ) act( AT_TEMP, message, rch, obj, NULL, TO_ROOM );
            if ( AT_TEMP >= 0 ) act( AT_TEMP, message, rch, obj, NULL, TO_CHAR );
	  }
...
void update_object_pulse ( void )
{
    ...
    for ( obj = last_object; obj; obj = oprev )
    {
        ...
        {
            if ( obj->value[4] <= 0 ) continue;

            if ( --obj->value[4] <= 0 ) explode( obj );
        ...
        else if ( obj->item_type == ITEM_REGENERATOR )
        {
            ...
            for ( rch = obj->in_room->first_person; rch; rch = rch->next_in_room )
            {
                ...
                if ( !IS_OBJ_STAT( obj, ITEM_ALIEN ) && rch->race == RACE_ALIEN ) continue;
                if ( !IS_OBJ_STAT( obj, ITEM_MARINE ) && rch->race == RACE_MARINE ) continue;
                if ( !IS_OBJ_STAT( obj, ITEM_PREDATOR ) && rch->race == RACE_PREDATOR ) continue;

                rch->hit = URANGE( 0, rch->hit + obj->value[2], rch->max_hit );

                if ( rch->race == RACE_ALIEN ) rch->resin = URANGE( 0, rch->resin + obj->value[3], get_max_resin(rch) );
                if ( rch->race == RACE_MARINE ) rch->morale = URANGE( 0, rch->morale + obj->value[3], get_max_morale(rch) );
                if ( rch->race == RACE_PREDATOR ) rch->field = URANGE( 0, rch->field + obj->value[3], rch->max_field );
        ...
        else if ( obj->item_type == ITEM_MOTIONB )
        {
            ...
            {                     
               ...
               if ( ( room = get_obj_room( obj ) ) == NULL ) continue;

               if ( room->area ) motion_ping( room->x, room->y, room->z, room->area, NULL );

               if ( obj->value[2] > 0 )
               {
                  if ( --obj->value[2] <= 0 ) extract_obj( obj );
               }
            }
        ...
        else if ( obj->item_type == ITEM_MSPAWNER )
        {
            ...
            if ( --obj->value[0] <= 0 )
            {
               ...
               {
                   if ( get_mob_index( obj->value[2] ) == NULL ) continue;
...
void update_respawn ( void )
{
    ...
    for ( ch = last_char; ch; ch = ch->prev )
    {
        ...
        if ( xIS_SET( ch->act, PLR_FREEZE) ) continue;
...
void update_movement()
{
     ...
     for ( ch = last_char; ch; ch = gch_prev )
     {
        if ( ch == first_char && ch->prev ) ch->prev = NULL;
        gch_prev = ch->prev;
        set_cur_char( ch );
        if ( gch_prev && gch_prev->next != ch )
        {
...
void recharge_drill( CHAR_DATA * ch, OBJ_DATA * obj )
{
   ...
   if ( obj == NULL ) return;
   if ( obj->first_content == NULL ) return;
...
void recharge_item( CHAR_DATA * ch, OBJ_DATA * obj )
{
   ...
   if ( obj->item_type == ITEM_LIGHT )
   {
      ...
      {
         if ( obj->value[2] < obj->value[4] ) obj->value[2]++;
      }
      else
      {
         if ( IS_SET( obj->value[3], BV00 ) )
   ...
   else if ( obj->item_type == ITEM_NVGOGGLE )
   {
      ...
      if ( !IS_SET( obj->value[3], BV00 ) )
      {
         if ( obj->value[0] < obj->value[1] ) obj->value[0]++;
      }
      else
      {
         if ( IS_SET( obj->value[3], BV00 ) )
