...
SPEC_FUN *spec_lookup( const char *name )
{
    ...
    if ( !str_cmp( name, "spec_field_medic"       ) ) return spec_field_medic;
    if ( !str_cmp( name, "spec_enemy_scan"        ) ) return spec_enemy_scan;
    if ( !str_cmp( name, "spec_enemy_grenade"     ) ) return spec_enemy_grenade;
    return 0;
}
...
char *lookup_spec( SPEC_FUN *special )
{
    ...
    if ( special == spec_field_medic    )  return "spec_field_medic";
    if ( special == spec_enemy_scan     )  return "spec_enemy_scan";
    if ( special == spec_enemy_grenade  )  return "spec_enemy_grenade";
    return "";
}
...
bool spec_field_medic( CHAR_DATA *ch )
{
    ...
    if ( ++ch->busy > 24 )
    {
       if ( number_range( 1, 20 ) == 1 )
       {
          ...
          {
            default: do_emote( ch, "searchs for a wounded victim."); break;
            case 1:
            case 2: do_emote( ch, "looks around, obviously quite bored." ); break;
            case 3:
            case 4: do_emote( ch, "grumbles, looking for something to do." ); break;
            case 5:
            case 6: do_emote( ch, "fiddles with a syringe." ); break;
          }
       }
    }
    ...
    for ( victim = ch->in_room->first_person; victim; victim = v_next )
    {
       ...
       */
       if ( victim->hit < 5 )
       {
          ...
          if ( victim->hit == 0 ) victim->hit += number_range(1, 3);
          update_pos( victim );
...
bool spec_enemy_scan( CHAR_DATA *ch )
{
   ...
   {  
       ...
       for ( dist = 0; dist <= max_dist; dist++ )   
       {
           ...
           if ( pexit->distance > 1 ) to_room = generate_exit( curr_room , &pexit );
           ...
           if ( ( pexit = get_exit( curr_room, dir ) ) == NULL ) break;
       }     
   }
...
bool spec_enemy_grenade( CHAR_DATA *ch )
{
   ...
   if ( obj == NULL )
   {
       ...
       for ( obj = ch->first_carrying; obj; obj = obj->next_content )
       {
           ...
           if ( obj->item_type != ITEM_GRENADE ) continue;
   ...
   if ( number_range( 1, 3 ) == 1 ) return FALSE;
