...
bool knows_player( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( !ch->pcdata || !victim->pcdata ) return TRUE;
    if ( IS_IMMORTAL( ch ) || IS_IMMORTAL( victim ) ) return TRUE;
...
char * g_name( CHAR_DATA * ch, CHAR_DATA * looker )
{
    ...
    if ( !looker ) return buf;
    if ( ch == looker ) return buf;
    if ( IS_NPC( ch ) || IS_NPC( looker ) ) return buf;
    if ( !ch->pcdata || !looker->pcdata ) return buf;
    if ( IS_IMMORTAL( ch ) || IS_IMMORTAL( looker ) ) return buf;

    if ( knows_player( looker, ch ) ) return buf;

    if ( !ch->pcdata->gname ) assign_gname( ch );
