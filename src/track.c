...
bool hail_route_check( CHAR_DATA *ch, ROOM_INDEX_DATA *toroom )
{
   ...
   switch(dir)
   {
      case BFS_ERROR:         return FALSE;
      case BFS_ALREADY_THERE: return FALSE;
      case BFS_NO_PATH:       return FALSE;
      default:                return TRUE;
   }
...
void hunt_victim( CHAR_DATA *ch )
{
   ...
   if ( !found )
   {
      if ( ch->race == RACE_MARINE ) do_emote( ch, "curses loudly." );
      if ( ch->race == RACE_ALIEN ) do_emote( ch, "screechs loudly!" );
      if ( ch->race == RACE_PREDATOR ) do_emote( ch, "clicks in anger." );
      stop_hunting( ch );
      return;
   }
   ...
   {
        if ( mob_snipe( ch, ch->hunting->who ) == TRUE ) return;
   }
   
   if ( xIS_SET( ch->act, ACT_SENTINEL ) ) return;
...
bool mob_snipe( CHAR_DATA *ch, CHAR_DATA *victim )
{
   ...
   if ( is_spectator( victim ) ) return FALSE;

   if ( ch->ap < get_max_ap( ch ) ) return FALSE;
   ...
   if ( xIS_SET( pexit->exit_info, EX_CLOSED ) ) continue;
   ...
   for ( dist = 0; dist <= max_dist; dist++ )   
   {
      ...
      to_room = NULL;
      if ( pexit->distance > 1 ) to_room = generate_exit( to_room , &pexit );
