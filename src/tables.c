...
DO_FUN* skill_function( char* name )
{
    switch ( name[3] )
    {
        case 'a':
            ...
            if ( !str_cmp( name, "do_allarenas" ) )
                return do_allarenas;

            if ( !str_cmp( name, "do_allbots" ) )
                return do_allbots;

            if ( !str_cmp( name, "do_allow" ) )
                return do_allow;

            if ( !str_cmp( name, "do_allowmp" ) )
                return do_allowmp;

            if ( !str_cmp( name, "do_allowpk" ) )
                return do_allowpk;
            ...
            if ( !str_cmp( name, "do_armor" ) )
                return do_armor;

            if ( !str_cmp( name, "do_abackup" ) )
                return do_abackup;
        ...
        case 'c':
            if ( !str_cmp( name, "do_carry" ) )
                return do_carry;

            if ( !str_cmp( name, "do_cache" ) )
                return do_cache;

            if ( !str_cmp( name, "do_cover" ) )
                return do_cover;

            if ( !str_cmp( name, "do_complain" ) )
                return do_complain;

            if ( !str_cmp( name, "do_compute" ) )
                return do_compute;
            ...
            if ( !str_cmp( name, "do_construct" ) )
                return do_construct;

            if ( !str_cmp( name, "do_confuse" ) )
                return do_confuse;
        ...
        case 'd':
            ...
            if ( !str_cmp( name, "do_dehive" ) )
                return do_dehive;

            if ( !str_cmp( name, "do_detach" ) )
                return do_detach;

            if ( !str_cmp( name, "do_deploy" ) )
                return do_deploy;
        ...
        case 'l':
            if ( !str_cmp( name, "do_lob" ) )
                return do_lob;

            if ( !str_cmp( name, "do_lunge" ) )
                return do_lunge;

            if ( !str_cmp( name, "do_lstat" ) )
                return do_lstat;
        ...
        case 'm':
            ...
            if ( !str_cmp( name, "do_makearena" ) )
                return do_makearena;

            if ( !str_cmp( name, "do_makeboard" ) )
                return do_makeboard;

            if ( !str_cmp( name, "do_makebot" ) )
                return do_makebot;
            ...
            if ( !str_cmp( name, "do_mpvar" ) )
                return do_mpvar;

            if ( !str_cmp( name, "do_mpmonitor" ) )
                return do_mpmonitor;

            if ( !str_cmp( name, "do_mpblast" ) )
                return do_mpblast;

            if ( !str_cmp( name, "do_mpcycle" ) )
                return do_mpcycle;

            if ( !str_cmp( name, "do_mpteamgain" ) )
                return do_mpteamgain;

            if ( !str_cmp( name, "do_mp_close_passage" ) )
                return do_mp_close_passage;

            if ( !str_cmp( name, "do_mpwait" ) )
                return do_mpwait;

            if ( !str_cmp( name, "do_mpadd" ) )
                return do_mpadd;

            if ( !str_cmp( name, "do_mpsub" ) )
                return do_mpsub;

            if ( !str_cmp( name, "do_mpset" ) )
                return do_mpset;

            if ( !str_cmp( name, "do_mprmflag" ) )
                return do_mprmflag;

            if ( !str_cmp( name, "do_mpreset" ) )
                return do_mpreset;

            if ( !str_cmp( name, "do_mprat" ) )
                return do_mprat;
        ...
        case 'p':
            ...
            if ( !str_cmp( name, "do_prestore" ) )
                return do_prestore;

            if ( !str_cmp( name, "do_prompt" ) )
                return do_prompt;

            if ( !str_cmp( name, "do_prone" ) )
                return do_prone;
        ...
        case 'r':
            ...
            if ( !str_cmp( name, "do_respawn" ) )
                return do_respawn;

            if ( !str_cmp( name, "do_rcreate" ) )
                return do_rcreate;

            if ( !str_cmp( name, "do_rage" ) )
                return do_rage;

            if ( !str_cmp( name, "do_radio" ) )
                return do_radio;

            if ( !str_cmp( name, "do_radio1" ) )
                return do_radio1;

            if ( !str_cmp( name, "do_radio2" ) )
                return do_radio2;

            if ( !str_cmp( name, "do_radio3" ) )
                return do_radio3;

            if ( !str_cmp( name, "do_radio4" ) )
                return do_radio4;
            ...
            if ( !str_cmp( name, "do_rcopy" ) )
                return do_rcopy;

            if ( !str_cmp( name, "do_rpcopy" ) )
                return do_rpcopy;
        ...
        case 's':
            if ( !str_cmp( name, "do_spit" ) )
                return do_spit;

            if ( !str_cmp( name, "do_smash" ) )
                return do_smash;
            ...
            if ( !str_cmp( name, "do_setmode" ) )
                return do_setmode;

            if ( !str_cmp( name, "do_setbot" ) )
                return do_setbot;

            if ( !str_cmp( name, "do_showbot" ) )
                return do_showbot;

            if ( !str_cmp( name, "do_setarena" ) )
                return do_setarena;

            if ( !str_cmp( name, "do_setlock" ) )
                return do_setlock;

            if ( !str_cmp( name, "do_setooc" ) )
                return do_setooc;

            if ( !str_cmp( name, "do_set_boot_time" ) )
                return do_set_boot_time;

            if ( !str_cmp( name, "do_showarena" ) )
                return do_showarena;
            ...
            if ( !str_cmp( name, "do_steacher" ) )
                return do_steacher;

            if ( !str_cmp( name, "do_stack" ) )
                return do_stack;

            if ( !str_cmp( name, "do_stand" ) )
                return do_stand;

            if ( !str_cmp( name, "do_spray" ) )
                return do_spray;
...
char* skill_name( DO_FUN* skill )
{
    ...
    if ( skill == do_allarenas )
        return "do_allarenas";

    if ( skill == do_allbots )
        return "do_allbots";

    if ( skill == do_makearena )
        return "do_makearena";

    if ( skill == do_showarena )
        return "do_showarena";

    if ( skill == do_setarena )
        return "do_setarena";
    ...
    if ( skill == do_mpmonitor )
        return "do_mpmonitor";

    if ( skill == do_mpteamgain )
        return "do_mpteamgain";

    if ( skill == do_mprmflag )
        return "do_mprmflag";
