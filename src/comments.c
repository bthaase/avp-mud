...
void do_comment( CHAR_DATA* ch, char* argument )
{
    ...
    if ( IS_NPC( ch ) )
    {
        send_to_char( "&zMobs can't use the comment command.\n\r", ch );
        return;
    }
    ...
    {
        send_to_char( "&zYou can't use the comment command from within the editor.\n\r", ch );
        return;
    }
    ...
    if ( !str_cmp( arg, "about" ) )
    {
        ...
        if ( !victim )
        {
            send_to_char( "&zThey're not logged on!\n\r", ch ); /* maybe fix this? */
            return;
        }

        if ( IS_NPC( victim ) )
        {
            send_to_char( "&zNo comments about mobs\n\r", ch );
            return;
        }
    }
    ...
    if ( !str_cmp( arg, "list" ) )
    {
        ...
        {
            send_to_char( "&zYou're not of the right caliber to do this...\n\r", ch );
            return;
        }

        if ( !victim->comments )
        {
            send_to_char( "&zThere are no relevant comments.\n\r", ch );
            return;
        }
    ...
    if ( !str_cmp( arg, "read" ) )
    {
        ...
        {
            send_to_char( "&zNote read which number?\n\r", ch );
            return;
        }
