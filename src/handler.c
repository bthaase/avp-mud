...
void room_explode_1( OBJ_DATA *obj , CHAR_DATA *xch, ROOM_INDEX_DATA *room , int blast, int range, int type )
{
    ...
    for ( rch = room->first_person ; rch ;  rch = rnext )
    {
       ...
       if ( !IS_NPC( rch ) )
       {
            if ( rch->pcdata && rch->hit > 0 )
            {
                ...
                if ( rch->pcdata->prepared[gsn_tumble] < skill_table[gsn_tumble]->reset ) tumble = 0;
         
                if ( xch == rch ) tumble = 0;

                if ( blast > ( rch->max_hit * 3 ) ) tumble = 0;

                if ( tumble > 0 ) tumble = ( 25 + ( tumble  * 25 ) );
       ...
       if ( xch != NULL ) { if ( !IS_NPC(xch) ) match_log( "GRENADE;'PC:%s' hit ':%s' with grenade blast.", xch->name, rch->name ); }
    ...
    {        
       ...
       if ( !obj_extracted(robj) ) robj->parent = rch;
    }        
...
int contents_value( OBJ_DATA * obj, int antiloop )
{
    ...
    if ( !obj->first_content ) return obj->cost;
...
int get_max_ap( CHAR_DATA *ch )
{
    ...
    {
       if ( ch->race == RACE_ALIEN ) ap = 2;
       if ( ch->race == RACE_MARINE ) ap = 3;
       if ( ch->race == RACE_PREDATOR ) ap = 3;
    }
...
int get_max_mp( CHAR_DATA *ch )
{
    ...
    if ( ch->race == RACE_ALIEN ) mp = 4;
    if ( ch->race == RACE_MARINE ) mp = 3;
    if ( ch->race == RACE_PREDATOR ) mp = 3;
...
int get_max_morale( CHAR_DATA *ch )
{
    ...
    if ( ch->race != RACE_MARINE ) return 0;
...
int get_max_resin( CHAR_DATA *ch )
{
    ...
    if ( ch->race != RACE_ALIEN ) return 0;
...
char * get_rank( int race, int level )
{
  ...
  if ( race == RACE_MARINE )
  {
     if ( level == 1 ) strcpy( buf, "Recruit" );
     if ( level >= 2 ) strcpy( buf, "Private" );
     if ( level >= 3 ) strcpy( buf, "Corporal" );
     if ( level >= 5 ) strcpy( buf, "Sergeant" );
     if ( level >= 8 ) strcpy( buf, "Lieutenant" );
     if ( level >= 11 ) strcpy( buf, "Captain" );
     if ( level >= 14 ) strcpy( buf, "Major" );
     if ( level >= 17 ) strcpy( buf, "Colonel" );
     if ( level >= 20 ) strcpy( buf, "General" );
  }
  else if ( race == RACE_ALIEN )
  {
     if ( level == 1 ) strcpy( buf, "Broodling" );
     if ( level >= 2 ) strcpy( buf, "Alien" );
     if ( level >= 3 ) strcpy( buf, "Scout" );
     if ( level >= 5 ) strcpy( buf, "Drone" );
     if ( level >= 8 ) strcpy( buf, "Sentry" );
     if ( level >= 11 ) strcpy( buf, "Hunter" );
     if ( level >= 14 ) strcpy( buf, "Warrior" );
     if ( level >= 17 ) strcpy( buf, "Guardian" );
     if ( level >= 20 ) strcpy( buf, "Queen" );
  }
  else if ( race == RACE_PREDATOR )
  {
     if ( level == 1 ) strcpy( buf, "Uninitiated" );
     if ( level >= 2 ) strcpy( buf, "Initiate" );
     if ( level >= 3 ) strcpy( buf, "Unblooded" );
     if ( level >= 5 ) strcpy( buf, "Blooded" );
     if ( level >= 8 ) strcpy( buf, "Student" );
     if ( level >= 11 ) strcpy( buf, "Warrior" );
     if ( level >= 14 ) strcpy( buf, "Champion" );
     if ( level >= 17 ) strcpy( buf, "Elder" );
     if ( level >= 20 ) strcpy( buf, "Ancient" );
  }
...
int can_carry_w( CHAR_DATA *ch )
{
    ...
    if ( ch->race == RACE_PREDATOR ) weight *= 2;
    if ( ch->race == RACE_ALIEN ) weight /= 2;
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
    ...
    case APPLY_STA:           ch->mod_sta               += mod; break;
    case APPLY_REC:           ch->mod_rec               += mod; break;
    case APPLY_INT:           ch->mod_int               += mod; break;
    case APPLY_BRA:           ch->mod_bra               += mod; break;
    case APPLY_PER:           ch->mod_per               += mod; break;
     ...
     */
    case APPLY_FIRE:          ch->protect[RIS_FIRE]     += mod; break;
    case APPLY_ENERGY:        ch->protect[RIS_ENERGY]   += mod; break;
    case APPLY_IMPACT:        ch->protect[RIS_IMPACT]   += mod; break;
    case APPLY_PIERCE:        ch->protect[RIS_PIERCE]   += mod; break;
    case APPLY_ACID:          ch->protect[RIS_ACID]     += mod; break;  
    }
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if ( ( !IS_NPC( ch ) || IS_BOT( ch ) ) )
    {
      if ( is_home( ch ) )
      {
         if ( !ch->was_home )
         {
            if ( IS_BOT(ch) ) { match_log( "EVENT;'BOT:%s' is home.", ch->name ); }
            else { match_log( "EVENT;'PC:%s' is home.", ch->name ); }
            ch->was_home = TRUE;
         }
      }
      else
      {
         if ( ch->was_home )
         {
            if ( IS_BOT(ch) ) { match_log( "EVENT;'BOT:%s' is deployed.", ch->name ); }
            else { match_log( "EVENT;'PC:%s' is deployed.", ch->name ); }
            ch->was_home = FALSE;
         }
      }
    }
   ...
   if ( ch->race == RACE_ALIEN ) update_swarm( );
...
int apply_ac( OBJ_DATA *obj, int iWear )
{
    ...
    case WEAR_OVER:     return 2 * obj->value[0];
...
OBJ_DATA *get_eq_char( CHAR_DATA *ch, int iWear )
{
    ...
    if ( ch->first_carrying == NULL ) return NULL;
...
void extract_char( CHAR_DATA *ch, bool fPull, bool menu )
{
    ...
    if ( !menu ) UNLINK( ch, first_char, last_char, next, prev );
...
CHAR_DATA *get_char_far_room( CHAR_DATA *ch, ROOM_INDEX_DATA * room, char *argument )
{
    ...
    for ( rch = room->first_person; rch; rch = rch->next_in_room )
    {
        ...
        if ( is_spectator( rch ) ) continue;
        if ( knows_player( ch, rch ) ) continue;
...
int get_obj_weight( OBJ_DATA *obj )
{
    ...
    if ( obj->attach )
    {
        ...
        if ( obj->attach->value[0] == 2 ) weight -= obj->attach->value[5];
        if ( obj->attach->value[0] == 4 ) weight -= obj->attach->value[4];
        if ( obj->attach->value[0] == 7 ) weight -= obj->attach->value[3];
        if ( obj->attach->value[0] == 8 ) weight -= obj->attach->value[3];
    }
...
bool room_is_dark( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if ( ch != NULL )
    {
        if ( ch->race == RACE_ALIEN ) return FALSE;

        if ( ch->pcdata )
        {
           if ( ch->pcdata->learned[gsn_nightvision] > 0 ) return FALSE;
        }

        if ( using_nvg( ch ) ) return FALSE;
    }
    ...
    {
        /* Night time :-D */
        if ( time_info.hour <  6 || time_info.hour > 19 ) return TRUE;
    }
...
{
    if ( xIS_SET( room->room_flags, ROOM_PROTOTYPE ) ) return FALSE;

    if ( xIS_SET( room->room_flags, ROOM_SAVEEQ ) ) return TRUE;
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( ch != NULL )
    {
        if ( ch->vent != victim->vent && !IS_IMMORTAL(ch) ) return FALSE;
    }
        ...
        if ( !IS_AFFECTED(victim, AFF_NAPALM) )
	{
          if ( (IS_AFFECTED(victim, AFF_CLOAK) && !IS_AFFECTED(victim, AFF_SHORTAGE)) && (ch->race != RACE_ALIEN) )
          {
            ...
            if ( !has_thermal( ch ) ) return FALSE;
          }
          ...
          if ( IS_AFFECTED(victim, AFF_HIDE) )
          {
            ...
            if ( IS_AFFECTED(ch, AFF_DETECT_HIDDEN) ) hide = FALSE;

            if ( !is_enemy( ch, victim ) ) hide = FALSE;
            if ( ch->race == RACE_ALIEN ) hide = FALSE;

            if ( victim->race == RACE_PREDATOR && ch->in_room != victim->in_room && has_thermal(ch) ) hide = FALSE;
...
bool can_see_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    */
    ...
    if ( ch->in_room && ch->race != RACE_ALIEN )
    {
       ...
       {
           if ( number_range( 1, 6 ) != 1 ) return FALSE;
       }
    }
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_STA:             return "stamina";
    case APPLY_REC:             return "recovery";
    case APPLY_INT:             return "intelligence";
    case APPLY_BRA:             return "bravery";
    case APPLY_PER:             return "perception";
...
char *affect_bit_name( EXT_BV *vector )
{
    ...
    if ( xIS_SET(*vector, AFF_NAPALM       )) strcat( buf, " napalm"        );
    ...
    if ( xIS_SET(*vector, AFF_SHORTAGE     )) strcat( buf, " shortage"      );
...
char *extra_bit_name( EXT_BV * extra_flags )
{
    ...
    if ( xIS_SET(*extra_flags, ITEM_HIDDEN)            ) strcat( buf, " hidden"    );
    if ( xIS_SET(*extra_flags, ITEM_COVERING)          ) strcat( buf, " covering"    );
    if ( xIS_SET(*extra_flags, ITEM_DEATHROT)          ) strcat( buf, " deathrot"     );
    if ( xIS_SET(*extra_flags, ITEM_BURRIED)           ) strcat( buf, " burried"    );
    if ( xIS_SET(*extra_flags, ITEM_PROTOTYPE)         ) strcat( buf, " prototype"    );
    if ( xIS_SET(*extra_flags, ITEM_NODUAL)            ) strcat( buf, " nodual"    );
    if ( xIS_SET(*extra_flags, ITEM_MARINE)            ) strcat( buf, " marine"    );
    if ( xIS_SET(*extra_flags, ITEM_PREDATOR)          ) strcat( buf, " predator"    );
    if ( xIS_SET(*extra_flags, ITEM_ALIEN)             ) strcat( buf, " alien"    );
...
bool char_died( CHAR_DATA *ch )
{
    ...
    /* Hack job */
    if ( is_spectator( ch ) ) return TRUE;
...
OBJ_DATA * group_object( OBJ_DATA *obj1, OBJ_DATA *obj2 )
{
    ...
    /* Explicit Ignores */
    if ( obj2->item_type == ITEM_TRAP ) return obj2;
    if ( obj2->item_type == ITEM_MSPAWNER ) return obj2;
    if ( obj2->item_type == ITEM_SENTRY ) return obj2;
...
bool checkclr( char *text, char *token )
{
    ...
    else
    {
        ...
        while (text[i] != '\0')
        {
            if ( text[i] == '&' && text[i+1] == token[0] ) return TRUE;
            i++;
        }
        return FALSE;
    }
...
bool is_spectator( CHAR_DATA * ch )
{
   ...
   if ( !ch->pcdata->spectator ) return FALSE;
...
int get_max_rounds( OBJ_DATA * obj )
{
   ...
   if ( obj->pIndexData ) return obj->pIndexData->value[2];

   if ( obj->value[2] > 0 ) return obj->value[2];
...
void drop_morale( CHAR_DATA * ch, int amount )
{
   ...
   if ( ch->race != RACE_MARINE ) return;

   if ( IS_IMMORTAL(ch) ) return;

   if ( ch->morale <= 0 ) nofreeze = TRUE;

   ch->morale = URANGE( 0, ch->morale - amount, get_max_morale( ch ) );

   if ( nofreeze ) return;

   if ( ch->morale <= 0 ) ch_printf( ch, "&Y(Alarm) Morale depleted. Good thing this isn't done, you lucky shmuck.\n\r" );
...
int get_variable( AREA_DATA * area, char * name )
{
  ...
  for ( var = area->first_var; var; var = var->next )
  {
     ...
     if ( !str_cmp( var->name, name ) ) return var->value;
  }
...
bool is_home( CHAR_DATA * ch )
{
  ...
  if ( ch->in_room->area == NULL ) return TRUE;

  area = ch->in_room->area;

  if ( ch->race == RACE_MARINE )
  {
      if ( !str_cmp( area->filename, "uscm_cc.are" ) ) return TRUE;
  }
  if ( ch->race == RACE_PREDATOR )
  {
      if ( !str_cmp( area->filename, "predator.are" ) ) return TRUE;
  }
  if ( ch->race == RACE_ALIEN )
  {
      if ( !str_cmp( area->filename, "hive.are" ) ) return TRUE;
  }

  if ( !str_cmp( area->filename, "limbo.are" ) ) return TRUE;
...
bool matching_ip( CHAR_DATA * ch, CHAR_DATA * gch )
{
    ...
    if ( ( dA = ch->desc ) != NULL )
    {
       if ( ( dB = gch->desc ) != NULL )
       {
           if ( !str_cmp( dA->host, dB->host ) ) return TRUE;               
       }
    }
...
{
  if ( !xIS_SET( room->room_flags, ROOM_INDOORS ) && !xIS_SET( room->room_flags, ROOM_UNDERGROUND ) ) return FALSE;

  if ( xIS_SET( room->room_flags, ROOM_NOHIVE ) ) return FALSE;
...
int get_dark_range( CHAR_DATA * ch )
{
  ...
  if ( ch->race == RACE_ALIEN ) return 999;

  for ( obj = ch->first_carrying; obj; obj = obj->next_content )
  {
     if ( obj->wear_loc == -1 ) continue;

     if ( !obj->attach ) continue;

     if ( obj->attach->value[0] == 6 ) override = obj->attach->value[2];
     
     if ( check_light_modifier( obj ) > 0 ) range = UMAX( 1, range );
  }

  if ( ( obj = get_eq_char( ch, WEAR_LIGHT ) ) != NULL )
  {
      if ( IS_SET(obj->value[3], BV00 ) ) range = UMAX( 1, range );
  }

  if ( override > 0 ) range = override;
...
int count_players( void )
{
    ...
    for ( d = last_descriptor; d; d = d->prev )
    {
        ...
        if ( IS_IDLE( d ) ) continue;

        if ( xIS_SET( ch->act, PLR_AFK ) ) continue;
...
bool using_nvg( CHAR_DATA * ch )
{
   ...
   for ( obj = ch->first_carrying; obj; obj = obj->next_content )
   {
      ...
      if ( obj->item_type == ITEM_NVGOGGLE && IS_SET( obj->value[3], BV00 ) ) return TRUE;
   }
