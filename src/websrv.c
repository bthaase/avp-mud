...
void handle_web_help_request(WEB_DESCRIPTOR *wdesc, int hmin, int hmax)
{
  ...
  for ( help = first_help; help; help = help->next )
  {      
     if ( help->level >= hmax || help->level <= hmin ) continue;
     ...
     {
          chk = help->keyword[1];
          if(help->keyword[0] == '"') inqt = 1;
     }
     else
     {
          if ( inqt == 1 ) send_buf( wdesc->fd, "<br><hr color=""#FFFFFF""><br>\n");
          inqt = 0;
     }
     ...
     {
          tmp = chk;
          if ( inqt == 1 ) sprintf( buf, "<br></font><font size=""4"">'%c'</font><font size=""2""><br>\n", tmp);
          if ( inqt == 0 ) sprintf( buf, "<br></font><font size=""4"">%c</font><font size=""2""><br>\n", tmp);
          send_buf( wdesc->fd, buf );
     }
  ...
  if ( cnt > 0 )
  {
     if(hmax >= 100) send_buf( wdesc->fd, "Links marked with a '*' means they are over level 100.<br>\n" );
