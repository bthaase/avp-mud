...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    // Must be at top
    if ( xIS_SET(obj->extra_flags, ITEM_RESPAWN) )   strcat( buf, "&w&B" );
    if ( obj->item_type == ITEM_AMMO ) 
    {
       if ( obj->value[2] <= 0 ) strcat( buf, "&w&R" );
    }
    if ( obj->item_type == ITEM_SPAWNER )
    {
       if ( obj->value[0] <= 0 ) strcat( buf, "&w&R" );
    }
    if ( sentry != NULL )                            strcat( buf, "&w&r(&RArmed&r) &G" );
    if ( obj->item_type == ITEM_COVER )              strcat( buf, "&w&z(&CCover&z) &G" );
    if ( obj->item_type == ITEM_FLARE )
      if ( obj->value[2] != 0 )                      strcat( buf, "(Burning) " );
    if ( xIS_SET(obj->extra_flags, ITEM_INVIS) )     strcat( buf, "(Invis) "     );
    if ( xIS_SET(obj->extra_flags, ITEM_ALIENINVIS)) strcat( buf, "(Alien Invis) " );
    if ( xIS_SET(obj->extra_flags, ITEM_GLOW) )      strcat( buf, "(Glowing) "   );
    if ( xIS_SET(obj->extra_flags, ITEM_HUM) )       strcat( buf, "(Humming) "   );
    if ( xIS_SET(obj->extra_flags, ITEM_HIDDEN) )    strcat( buf, "(Hidden) "    );
    if ( xIS_SET(obj->extra_flags, ITEM_BURRIED) )   strcat( buf, "(Burried) "   );
    if ( IS_IMMORTAL(ch) )
    {
       if ( xIS_SET(obj->extra_flags, ITEM_PROTOTYPE) ) strcat( buf, "(Prototype)" );
    }
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    {
       if ( victim->pIndexData->vnum == 1 ) return;  /* Puff */
       if ( victim->pIndexData->vnum == 3 ) return;  /* Supermob */
    }
    ...
    {
      if ( victim->race == RACE_ALIEN ) strcpy( prefix, "&w&z[&WAlien&z] " );
      if ( victim->race == RACE_MARINE ) strcpy( prefix, "&w&z[&WMarine&z] " );
      if ( victim->race == RACE_PREDATOR ) strcpy( prefix, "&w&z[&WPredator&z] " );
    }
    ...
    if ( IN_VENT(victim) && IS_IMMORTAL(ch)   ) strcat( buf, "(Vent) "       );
    if ( IS_AFFECTED(victim, AFF_INVISIBLE)   ) strcat( buf, "(Invis) "      );
    if ( IS_AFFECTED(victim, AFF_HIDE)        ) strcat( buf, "(Hide) "       );
    if ( IS_AFFECTED(victim, AFF_BLIND)       ) strcat( buf, "(Blinded) "    );
    if ( victim->block != NULL                ) strcat( buf, "(Blocking) "   );
    if ( get_timerptr( victim, TIMER_DO_FUN ) ) strcat( buf, "(Working) "   );
    if ( IS_AFFECTED(victim, AFF_CLOAK)       )
    {
      if ( IS_AFFECTED(victim, AFF_SHORTAGE) )  strcat( buf, "(Decloaked) "    );
      if ( !IS_AFFECTED(victim, AFF_SHORTAGE) ) strcat( buf, "(Cloaked) "    );
    }
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    else
    {
	    ...
	    send_to_char( format_obj_to_char( obj, ch, TRUE ), ch );
            ...
            for ( iWear = 0; iWear < MAX_WEAR; iWear++ )
            {
               if ( iWear != WEAR_WIELD && iWear != WEAR_DUAL_WIELD && iWear != WEAR_SHIELD ) continue;
               if ( ( obj = get_eq_char( victim, iWear ) ) != NULL && can_see_obj( ch, obj ) )
               {
                   send_to_char( "&B ", ch );
                   send_to_char( where_name[iWear], ch );
	...
	send_to_char(" ", ch);
        ...
        if ( arg1[0] == '\0' || ( !IS_NPC(ch) && !xIS_SET(ch->act, PLR_BRIEF) ) )
        {
           if ( IN_VENT( ch ) )
           {
                ...
                if ( tmp != NULL ) send_to_char( tmp->description, ch );
           }
    ...
    if ( !str_cmp( arg1, "out" ) )
    {
        if ( ch->vent && ch->in_room )
        {
                ...
                xIS_SET( ch->in_room->room_flags, ROOM_VENTED_D ) )
           {
              ch->vent = FALSE;
              do_look( ch, "" );
              if ( !is_spectator(ch) ) run_awareness( ch->in_room->first_person, ch );
              ch->vent = TRUE;
           }
           else
           {
...
void show_obj_desc( CHAR_DATA * ch, OBJ_DATA * obj )
{
    ...
    {
       pdesc = get_extra_descr( obj->name, obj->pIndexData->first_extradesc );
       if ( !pdesc ) pdesc = get_extra_descr( obj->name, obj->first_extradesc );
       if ( !pdesc )
            send_to_char( "You can't recall anything about this item.\r\n", ch );
       else
            send_to_char( pdesc, ch );
...
void show_condition( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    else if ( percent >=  50 ) strcat( buf, " has many broken pieces.\n\r" );
...
void do_examine( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '#' )
    {
        ...
        { 
            if ( obj->wear_loc == WEAR_NONE && can_see_obj( ch, obj ) ) onum++;
            if ( onum == oref ) 
            {
    ...
    {
        if ( ( obj = get_obj_here( ch, arg ) ) != NULL ) examine_obj( ch, obj );
    }
...
void examine_obj( CHAR_DATA * ch, OBJ_DATA * obj )
{
	...
	case ITEM_ARMOR:
            ...
            if ( obj->value[3] == 1 ) ch_printf( ch, "&zArmor class: &Clight.\n\r" );
            if ( obj->value[3] == 2 ) ch_printf( ch, "&zArmor class: &Cmedium.\n\r" );
            if ( obj->value[3] == 3 ) ch_printf( ch, "&zArmor class: &Cheavy.\n\r" );
	    break;
        ...
        case ITEM_GRENADE:
            ...
            if ( obj->value[3] >= 0 && obj->value[3] <= 4 ) sprintf( buf, "%s", ris_flags[obj->value[3]] );
	...
	case ITEM_WEAPON:
            ...
            {
               default: return;
               case WEAPON_KNIFE:        strcpy( buf, "Knife Weapon (Marine-Class)\n\r" ); break;
               case WEAPON_PISTOL:       strcpy( buf, "Pistol Weapon (Marine-Class)\n\r" ); break;
               case WEAPON_RIFLE:        strcpy( buf, "Rifle Weapon (Marine-Class)\n\r" ); break;
               case WEAPON_SHOTGUN:      strcpy( buf, "Shotgun Weapon (Marine-Class)\n\r" ); break;
               case WEAPON_AUTOMATIC:    strcpy( buf, "Automatic Weapon (Marine-Class)\n\r" ); break;
               case WEAPON_FLAMETHROWER: strcpy( buf, "Flamethrower Weapon (Marine-Class)\n\r" ); break;
               case WEAPON_ROCKETFIRED:  strcpy( buf, "Projectile-based Weapon (Marine-Class)\n\r" ); break;
               case WEAPON_BLADE:        strcpy( buf, "Blade Weapon (Predator-Class)\n\r" ); break;
               case WEAPON_SPEAR:        strcpy( buf, "Spear Weapon (Predator-Class)\n\r" ); break;
               case WEAPON_ERANGED:      strcpy( buf, "Ranged Energy Weapon (Predator-Class)\n\r" ); break;
               case WEAPON_RANGED:       strcpy( buf, "Ranged Weapon (Predator-Class)\n\r" ); break;
               case WEAPON_DISC:         strcpy( buf, "Disc Weapon (Predator-Class)\n\r" ); break;
            }
            send_to_char( buf, ch );
            ...
            {
              strcpy( buf, "" );
              if ( obj->value[4] >= 0 && obj->value[4] <= 4 ) sprintf( buf, "%s ", ris_flags[obj->value[4]] );
            ...
            if ( obj->ammo )
            {
                 ...
                 {
                    if ( obj->ammo->value[0] >= 0 && obj->ammo->value[0] <= 4 ) sprintf( buf, "%s", ris_flags[obj->ammo->value[0]] );
        ...
        case ITEM_AMMO:
            strcpy( buf, "Unknown Type" );
            if ( obj->value[0] >= 0 && obj->value[0] <= 4 ) sprintf( buf, "%s", ris_flags[obj->value[0]] );
...
void do_exits( CHAR_DATA *ch, char *argument )
{
        ...
        if ( pexit->to_room && !xIS_SET(pexit->exit_info, EX_HIDDEN) && ( !xIS_SET(pexit->exit_info, EX_BLASTOPEN) || xIS_SET( pexit->exit_info, EX_BLASTED ) ) && ( !xIS_SET(pexit->exit_info, EX_NOVENT) || !ch->vent ) )
	{
	    ...
	    {
                sign = '-';
                if ( xIS_SET(pexit->exit_info, EX_ISDOOR) ) sign = '+';
...
void do_help( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
       if ( ch->race == RACE_ALIEN ) pHelp = get_help( ch, "Alien Index" );
       if ( ch->race == RACE_MARINE ) pHelp = get_help( ch, "Marine Index" );
       if ( ch->race == RACE_PREDATOR ) pHelp = get_help( ch, "Predator Index" );
    }
    else
    {
    ...
    if ( pHelp == NULL )
    {
        ...
        /* Log Lookup Attempt */
        {
            ...
            if ( file_size(filename) > 500000 ) return;
...
void do_who( CHAR_DATA *ch, char *argument )
{
	...
	char const *race;
        ...
        if ( bTmp->race == RACE_ALIEN ) cur_who->type = WT_ALIEN;
        else if ( bTmp->race == RACE_MARINE ) cur_who->type = WT_MARINE;
        else cur_who->type = WT_PREDATOR;
	...
	|| d->original)
        ...
        CREATE( cur_who, WHO_DATA, 1 );
        cur_who->text = str_dup( buf );
        if ( wch->race == RACE_ALIEN ) cur_who->type = WT_ALIEN;
        else if ( wch->race == RACE_MARINE ) cur_who->type = WT_MARINE;
        else cur_who->type = WT_PREDATOR;
...
void do_where( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "full" ) && IS_IMMORTAL( ch ) ) full = TRUE;
    ...
    if ( arg[0] == '\0' || full )
    {
        if ( full ) send_to_pager( "Players logged in:\n\r", ch );
        if ( !full ) pager_printf( ch, "Players near you in %s:\n\r", ch->in_room->area->name );
	found = FALSE;
	for ( d = first_descriptor; d; d = d->next )
	    if ( (d->connected == CON_PLAYING || d->connected == CON_EDITING )
	    && ( victim = d->character ) != NULL
...
void do_consider( CHAR_DATA *ch, char *argument )
{
    ...
    diff = ch->hit * 100 / UMAX( 1, victim->hit );
         if ( diff <=  10 ) {msg = "$E is currently far healthier than you are."; overall -= 3;}
    else if ( diff <=  50 ) {msg = "$E is currently much healthier than you are."; overall -= 2;}
    else if ( diff <=  75 ) {msg = "$E is currently slightly healthier than you are."; overall -= 1;}
    else if ( diff <= 125 ) {msg = "$E is currently about as healthy as you are.";}
    else if ( diff <= 200 ) {msg = "You are currently slightly healthier than $M."; overall += 1;}
    else if ( diff <= 500 ) {msg = "You are currently much healthier than $M."; overall += 2;}
    else                    {msg = "You are currently far healthier than $M."; overall += 3;}
    if ( full ) act(AT_LBLUE, msg, ch, NULL, victim, TO_CHAR);

    {
         ...
         if ( wA )
         {
              ...
              if ( (wA = get_eq_char( ch, WEAR_DUAL_WIELD )) != NULL ) diff -= ( (wA->value[1]*10) + (wA->value[2]*10) ) / 2;
         }

         if ( wB )
         {
              diff += ( (wB->value[1]*10) + (wB->value[2]*10) ) / 2;
              if ( (wB = get_eq_char( victim, WEAR_DUAL_WIELD )) != NULL ) diff += ( (wB->value[1]*10) + (wB->value[2]*10) ) / 2;
         }
    
              if ( diff <= -25 ) {msg = "You are far better armed than $M."; overall += 3;}
         else if ( diff <= -12 ) {msg = "$E is not as well armed as you are."; overall += 2;}
         else if ( diff <=  -5 ) {msg = "$E doesn't seem quite as well armed as you."; overall += 1;}
         else if ( diff <=   5 ) {msg = "You are about as well armed as $M.";}
         else if ( diff <=  12 ) {msg = "$E is slightly better armed than you are."; overall -= 1;}
         else if ( diff <=  25 ) {msg = "$E seems to be much better armed than you are."; overall -= 2;}
         else                    {msg = "$E is far better armed than you are."; overall -= 3;}
         if ( full ) act(AT_LBLUE, msg, ch, NULL, victim, TO_CHAR);
    }

    diff = overall;
         if ( diff <= -10 ) {msg = "Conclusion - You wouldn't last more than a few seconds.";}
    else if ( diff <=  -7 ) {msg = "Conclusion - You would need a lot of luck to beat $M.";}
    else if ( diff <=  -3 ) {msg = "Conclusion - You would need some luck to beat $M.";}
    ...
    else if ( diff <=   9 ) {msg = "Conclusion - $N is no match for you. You could easily beat $M.";}
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	{
            if ( sn >= top_sn ) { sn = 0; clevel++; continue; }

            if ( !skill_table[sn]->name ) break;
            if ( skill_table[sn]->min_level != clevel ) continue;
            if ( skill_table[sn]->race < 0 || skill_table[sn]->race > 2 ) continue;
            if ( skill_table[sn]->race != ch->race ) continue;
            if ( ch->pcdata->learned[sn] <= 0 ) continue;

            ++cnt;

            if ( ch->pcdata->learned[sn] <= 1 ) strcpy( buf, "Basic" );
            if ( ch->pcdata->learned[sn] == 2 ) strcpy( buf, "Advanced" );
            if ( ch->pcdata->learned[sn] >= 3 ) strcpy( buf, "Expert" );
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
        ...
        else if ( !str_prefix( arg+1, "censor"   ) ) bit = PLR_CENSOR;
        else if ( !str_prefix( arg+1, "compact"  ) ) bit = PLR_COMPACT;
        else if ( !str_prefix( arg+1, "whois"    ) ) bit = PLR_WHOIS;
	...
	if (bit)
	  ...
	  send_to_char( "Ok.\n\r", ch );
        ...
        else
        {
          ...
          else if ( !str_prefix( arg+1, "showreset" ) ) bit = PCFLAG_SHOWRESET;
          else if ( !str_prefix( arg+1, "showammo" ) ) bit = PCFLAG_SHOWAMMO;
          else if ( !str_prefix( arg+1, "nowho"    )
                    && (IS_IMMORTAL(ch))) bit = PCFLAG_NOWHO;
...
void HTML_Objstats( void )
{
  ...
  for ( area = first_area ; area ; area = area->next )
  {
      ...
      {
        if ( ( pObjIndex = get_obj_index( vnum ) ) == NULL ) continue;
        fprintf( fp, "<tr><td>\n" );
...
void HTML_Ammostats( void )
{
  ...
  for ( area = first_asort; area; area = area->next_sort )
  {
      ...
      {
        if ( ( pObjIndex = get_obj_index( vnum ) ) == NULL ) continue;
        if ( pObjIndex->item_type != ITEM_AMMO ) continue;
        fprintf( fp, "<tr><td>\n" );
        fprintf( fp, "%s", conv_hcolor( pObjIndex->short_descr ) );
        fprintf( fp, "</td><td>\n");
        fprintf( fp, "%d", pObjIndex->vnum );
  ...
  for ( area = first_asort; area; area = area->next_sort )
  {
      ...
      {
        if ( ( pObjIndex = get_obj_index( vnum ) ) == NULL ) continue;
        if ( pObjIndex->item_type != ITEM_WEAPON ) continue;
        if ( pObjIndex->value[0] == WEAPON_KNIFE || pObjIndex->value[0] == WEAPON_SPEAR
         ...
         || pObjIndex->value[0] == WEAPON_DISC || pObjIndex->value[0] == WEAPON_NATURAL ) continue;
        fprintf( fp, "<tr><td>\n" );
        fprintf( fp, "%s", conv_hcolor( pObjIndex->short_descr ) );
        fprintf( fp, "</td><td>\n");
        fprintf( fp, "%d", pObjIndex->vnum );
  ...
  while ( *msg != 0 )
  {
    ...
    else if ( *msg == '&' )
    {
        ...
        */
         switch( *msg )
         {
           case 'x': strcat(conv_result, "<font color=#000000>"); break;  /* Black */
           case 'g': strcat(conv_result, "<font color=#008000>"); break;  /* Green */
           case 'b': strcat(conv_result, "<font color=#000080>"); break;  /* Dark Blue */
           case 'c': strcat(conv_result, "<font color=#408080>"); break;  /* Cyan */
           case 'z': strcat(conv_result, "<font color=#669999>"); break;  /* Dark Grey */
           case 'G': strcat(conv_result, "<font color=#00FF00>"); break;  /* Light Green */
           case 'B': strcat(conv_result, "<font color=#0000FF>"); break;  /* Blue */
           case 'C': strcat(conv_result, "<font color=#00FFFF>"); break;  /* Light Blue */
           case 'r': strcat(conv_result, "<font color=#800000>"); break;  /* Blood Red */
           case 'O': strcat(conv_result, "<font color=#804000>"); break;  /* Orange(Brown) */
           case 'p': strcat(conv_result, "<font color=#FF00FF>"); break;  /* Purple */
           case 'w': strcat(conv_result, "<font color=#FFFFFF>"); break;  /* Gray(White?) */
           case 'R': strcat(conv_result, "<font color=#FF0000>"); break;  /* Light Red */
           case 'Y': strcat(conv_result, "<font color=#FFFF00>"); break;  /* Yellow */
           case 'P': strcat(conv_result, "<font color=#FF00FF>"); break;  /* Pink */
           case 'W': strcat(conv_result, "<font color=#C0C0C0>"); break;  /* White(Grey?) */
         }
...
void send_skill_store( CHAR_DATA * ch, DESCRIPTOR_DATA * d )
{
    ...
    if ( ch->race == RACE_ALIEN ) { strcpy( cA, "&B" ); strcpy( cB, "&b" ); }
    if ( ch->race == RACE_MARINE ) { strcpy( cA, "&R" ); strcpy( cB, "&r" ); }
    if ( ch->race == RACE_PREDATOR ) { strcpy( cA, "&G" ); strcpy( cB, "&g" ); }
...
void map_area( AREA_DATA * area, CHAR_DATA * ch, char * filename )
{
    ...
    for ( vnum = area->low_r_vnum; vnum <= area->hi_r_vnum; vnum++ )
    {
       ...
       if ( xIS_SET( room->room_flags, ROOM_NOCOORD ) ) continue;
