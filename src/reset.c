  ...
  if ( !str_prefix(arg, "object") )
  {
    ...
    if ( !str_prefix( arg, "put" ) )
    {
      ...
      if ( !(reset = find_oreset(ch, pArea, aRoom, arg)) )
        return;
      /* Put in_objects after hide resets */
...
void add_obj_reset( AREA_DATA *pArea, char cm, OBJ_DATA *obj, int v2, int v3 )
{
  ...
  if ( cm == 'O' && IS_OBJ_STAT(obj, ITEM_HIDDEN) &&
      !xIS_SET(obj->wear_flags, ITEM_TAKE) )
...
void instaroom( AREA_DATA *pArea, ROOM_INDEX_DATA *pRoom, bool dodoors )
{
  ...
  if ( dodoors )
  {
    ...
    for ( pexit = pRoom->first_exit; pexit; pexit = pexit->next )
    {
      ...
      if ( !xIS_SET( pexit->exit_info, EX_ISDOOR ) && !xIS_SET( pexit->exit_info, EX_BLASTOPEN ) )
        continue;
      ...
      else if ( xIS_SET( pexit->exit_info, EX_BLASTOPEN ) ) state = 3;
...
void reset_area( AREA_DATA *pArea )
{
  ...
  for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
  {
    ...
    case 'M':
      ...
      {
        bug( "Reset_area: 'M': bad mob vnum %d in %s.", pReset->arg1, pArea->name );
        continue;
      }
      if ( !(pRoomIndex = get_room_index(pReset->arg3)) )
      {
        bug( "Reset_area: 'M': bad room vnum %d in %s.", pReset->arg3, pArea->name );
        continue;
      }
      ...
      // if ( !xIS_EMPTY(mob->vip_flags) && pArea->planet )
    ...
    case 'E':
      ...
      {
        bug( "Reset_area: 'E' or 'G': bad obj vnum %d in %s.", pReset->arg1, pArea->name );
        continue;
      }
      if ( !mob )
      {
    ...
    case 'O':
      if ( !(pObjIndex = get_obj_index(pReset->arg1)) )
      {
        bug( "Reset_area: 'O': bad obj vnum %d in %s.", pReset->arg1, pArea->name );
        continue;
      }
      if ( !(pRoomIndex = get_room_index(pReset->arg3)) )
      {
        bug( "Reset_area: 'O': bad room vnum %d in %s.", pReset->arg3, pArea->name );
        continue;
      }
    ...
    case 'P':
      if ( !(pObjIndex = get_obj_index(pReset->arg1)) )
      {
        bug( "Reset_area: 'P': bad obj vnum %d in %s.", pReset->arg1, pArea->name );
        continue;
      }
      if ( pReset->arg3 > 0 )
      {
        ...
        {
          bug( "Reset_area: 'P': bad objto vnum %d in %s.", pReset->arg3, pArea->name );
          continue;
        }
        if ( pArea->nplayer > 0 ||
    ...
    case 'B':
      ...
      case BIT_RESET_DOOR:
        ...
        {
          bug( "Reset_area: 'B': door: bad room vnum %d in %s.", pReset->arg1, pArea->name );
          continue;
        }
      ...
      case BIT_RESET_ROOM:
        if ( !(pRoomIndex = get_room_index(pReset->arg1)) )
        {
          bug( "Reset_area: 'B': room: bad room vnum %d in %s.", pReset->arg1, pArea->name );
          continue;
        }
      ...
      case BIT_RESET_MOBILE:
        ...
        break;
      default:
        bug( "Reset_area: 'B': bad options %d.", pReset->arg2 );
        continue;
      }
      if ( IS_SET(pReset->arg2, BIT_RESET_SET) )
        xSET_BIT(*plc, pReset->arg3);
      else if ( IS_SET(pReset->arg2, BIT_RESET_TOGGLE) )
        xTOGGLE_BIT(*plc, pReset->arg3);
      else
        xREMOVE_BIT(*plc, pReset->arg3);
      break;
    
    case 'D':
      if ( !(pRoomIndex = get_room_index(pReset->arg1)) )
      {
        bug( "Reset_area: 'D': bad room vnum %d in %s.", pReset->arg1, pArea->name );
        continue;
      }
      ...
      case 2:
        ...
        if ( xIS_SET(pexit->exit_info, EX_xSEARCHABLE) )
          xSET_BIT( pexit->exit_info, EX_SECRET);
        break;
      case 3: // Patch for 'blastable' exits
        xREMOVE_BIT( pexit->exit_info, EX_BLASTED );
        break;
      }
      break;
    ...
    case 'R':
      if ( !(pRoomIndex = get_room_index(pReset->arg1)) )
      {
        bug( "Reset_area: 'R': bad room vnum %d.", pReset->arg1 );
        continue;
      }
    ...
    case 'B':
      ...
      if ( flagarray )
        sprintf(pbuf, "; flags: %s [%d]\n\r",
            old_flag_string(pReset->arg3, flagarray), pReset->arg3);
      else
