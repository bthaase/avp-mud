...
{   
    ...
    if ( xIS_EMPTY( room->hroom_flags ) ) return;
...
void do_goto( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg2, "-silent" ) ) silent = TRUE;
    ...
    if ( ( location = find_location( ch, arg ) ) == NULL )
    {
        ...
        {
             if ( !silent ) send_to_char( "You cannot find that...\n\r", ch );
             return;
        }
	
	if ( vnum < 1 || IS_NPC(ch) || !ch->pcdata->area )
	{
             if ( !silent ) send_to_char( "No such location.\n\r", ch );
             return;
	}
	if ( get_trust( ch ) < sysdata.level_modify_proto )
	{
             if ( !ch->pcdata || !(pArea=ch->pcdata->area) )
             {
                 if ( !silent ) send_to_char( "You must have an assigned area to create rooms.\n\r", ch );
                 return;
             }
             if ( vnum < pArea->low_r_vnum || vnum > pArea->hi_r_vnum )
             {
                 if ( !silent ) send_to_char( "That room is not within your assigned range.\n\r", ch );
                 return;
             }
	}
    ...
    if ( get_trust( ch ) < LEVEL_IMMORTAL )
    {
        ...
        if ( !ch->pcdata || !(pArea=ch->pcdata->area) )
        {
           if ( !silent ) send_to_char( "You must have an assigned area to goto.\n\r", ch );
	   return;
        }
	...
	{
            if ( !silent ) send_to_char( "Builders can only use goto from a hotel or in their zone.\n\r", ch );
            return;
	}
	...
	{
            if ( !silent ) act( AT_ACTION, "You follow $N.", fch, NULL, ch, TO_CHAR );
...
*/
	...
	while ( argument[0] != '\0' )
	{
	   ...
	   else
	   {
              ...
              {
                 if ( value == ROOM_HIVED ) hflag_toggle( location );                 
...
void do_astat( CHAR_DATA *ch, char *argument )
{
    ...
    for ( vnum = tarea->low_r_vnum ; vnum <= tarea->hi_r_vnum ; vnum++ )
    {
      ...
      {
        if ( xIS_SET( room->room_flags, ROOM_VENTED_A ) ) vents++;
        else if ( xIS_SET( room->room_flags, ROOM_VENTED_B ) ) vents++;
        else if ( xIS_SET( room->room_flags, ROOM_VENTED_C ) ) vents++;
        else if ( xIS_SET( room->room_flags, ROOM_VENTED_D ) ) vents++;
        used_vnum++;
      }
    }
...
void do_aset( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if ( !str_cmp( argument, "low" ) ) tarea->ambience = 1;
        else if ( !str_cmp( argument, "medium" ) ) tarea->ambience = 2;
        else if ( !str_cmp( argument, "high" ) ) tarea->ambience = 3;
        else
        {
            send_to_char( "aset (area name) ambience [low/medium/high]\n\r", ch );
            return;
        }
        if ( tarea->ambience == 1 ) send_to_char( "Ambience level set to LOW. (YELL Range is 6 rooms)\n\r", ch );
        if ( tarea->ambience == 2 ) send_to_char( "Ambience level set to MEDIUM. (YELL Range is 4 rooms)\n\r", ch );
        if ( tarea->ambience == 3 ) send_to_char( "Ambience level set to HIGH. (YELL Range is 2 rooms)\n\r", ch );
        return;
    }
	...
	if ( (room = get_room_index( vnum )) == NULL )
        {
          ...
          if ( argument[0] != '\0' ) room->name = STRALLOC(argument);
          cnt++;
          continue;
        }
    }                                                   
...
void do_rcopy( CHAR_DATA *ch, char *argument )
{
    ...
    if ( target->name ) STRFREE( target->name );
    if ( target->description ) STRFREE( target->description );
    if ( target->hdescription ) STRFREE( target->hdescription );
...
void do_rpcopy( CHAR_DATA *ch, char *argument )
{
    ...
    if ( fault > 0 ) ch_printf( ch, "&R%d Program(s) failed to copy to the target location.\n\r", fault );
...
int free_area_vnum( CHAR_DATA * ch )
{
    ...
    if ( !ch->pcdata ) return -1;
    if ( !ch->pcdata->area ) return -1;
    ...
    {
       if ( get_room_index( i ) == NULL ) return i;
    }
