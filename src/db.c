...
void area_update( void )
{
	...
	if ( pArea->nplayer == 0 || pArea->age >= reset_age )
	{
	    ROOM_INDEX_DATA *pRoomIndex;
            ...
            if ( !xIS_SET(pArea->flags, AFLAG_NORESET ) ) reset_area( pArea );
	    if ( reset_age == -1 )
		pArea->age = -1;
	    else
...
void free_char( CHAR_DATA *ch )
{
	...
	STRFREE( ch->pcdata->bio	);
        if ( ch->pcdata->gname ) DISPOSE( ch->pcdata->gname );  /* no hash */
        if ( ch->pcdata->fname ) DISPOSE( ch->pcdata->fname );  /* no hash */
        if ( ch->pcdata->lname ) DISPOSE( ch->pcdata->lname );  /* no hash */
  ...
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
   if ( wiz->level > LEVEL_AVATAR )
   {
	...
	buf[0] = '\0';
      ...
      switch(ilevel)
      {
        case MWIZ_LEVEL -  0: towizfile( "     &GThe Chief Executives&W" );  break;
        case MAX_LEVEL -  0:  towizfile( "     &cThe Chairman of the Board&W" );            break;
        case MAX_LEVEL -  1:  towizfile( "     &CThe Defense Attorney&W" );     break;
        case MAX_LEVEL -  2:  towizfile( " The Employees" );               break;
        case MAX_LEVEL -  3:  towizfile( " The Consultants" );        break;
        default:              towizfile( " The Stockholders" );           break;
      }
    }
...
int mprog_name_to_type ( char *name )
{
   ...
   if ( !str_cmp( name, "useon_prog"     ) )    return USEON_PROG;
   if ( !str_cmp( name, "useoff_prog"     ) )   return USEOFF_PROG;
   ...
   if ( !str_cmp( name, "pulse_prog"    ) )     return PULSE_PROG;
   return( ERROR_PROG );
}
...
bool delete_obj( OBJ_INDEX_DATA *obj )
{
  ...
  while ((ed = obj->first_extradesc) != NULL)
  {
    ...
    if ( ed->keyword ) STRFREE(ed->keyword);
    if ( ed->description ) STRFREE(ed->description);
    DISPOSE(ed);
    --top_ed;
  }
  ...
  if ( obj->name ) STRFREE(obj->name);
  if ( obj->short_descr ) STRFREE(obj->short_descr);
  if ( obj->description ) STRFREE(obj->description);
  if ( obj->action_desc ) STRFREE(obj->action_desc);
...
/*    FILE *fpin;
	...
	else if ( !str_cmp( word, "AREA"     ) )
	{
        ...
        else if ( !str_cmp( word, "AMBIENCE" ) ) load_ambience(tarea, fpArea);
...
void load_mplist(void)
{
  ...
  for ( ; ; )
  {
    ...
    if(!(line = fread_string( fp ))) return;
    // perror(line);
    if(strstr(line,"###")) return;
...
void deploy_map( void )
{
   ...
   for ( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
   {
       ...
       {
            if ( xIS_SET( room->room_flags, ROOM_MAPSTART ) ) map_apply_coords( room, 0, 0, 0 );
       }
   }
          ...
          if ( pexit->to_room && pexit->to_room != room )
          {
              ...
              if ( xIS_SET( pexit->to_room->room_flags, BFS_MARK ) )
              {
                  ...
                  switch( pexit->vdir )
                  {
                     ...
                     case 0: if ( (pexit->to_room->x == (x + 0)) && (pexit->to_room->y == (y + 1)) && (pexit->to_room->z == (z + 0)) ) { safe = TRUE; } break;
                     case 1: if ( (pexit->to_room->x == (x + 1)) && (pexit->to_room->y == (y + 0)) && (pexit->to_room->z == (z + 0)) ) { safe = TRUE; } break;
                     case 2: if ( (pexit->to_room->x == (x + 0)) && (pexit->to_room->y == (y - 1)) && (pexit->to_room->z == (z + 0)) ) { safe = TRUE; } break;
                     case 3: if ( (pexit->to_room->x == (x - 1)) && (pexit->to_room->y == (y + 0)) && (pexit->to_room->z == (z + 0)) ) { safe = TRUE; } break;
                     case 4: if ( (pexit->to_room->x == (x + 0)) && (pexit->to_room->y == (y + 0)) && (pexit->to_room->z == (z + 1)) ) { safe = TRUE; } break;
                     case 5: if ( (pexit->to_room->x == (x + 0)) && (pexit->to_room->y == (y + 0)) && (pexit->to_room->z == (z - 1)) ) { safe = TRUE; } break;
                     case 6: if ( (pexit->to_room->x == (x + 1)) && (pexit->to_room->y == (y + 1)) && (pexit->to_room->z == (z + 0)) ) { safe = TRUE; } break;
                     case 7: if ( (pexit->to_room->x == (x - 1)) && (pexit->to_room->y == (y + 1)) && (pexit->to_room->z == (z + 0)) ) { safe = TRUE; } break;                                                                                                                
                     case 8: if ( (pexit->to_room->x == (x + 1)) && (pexit->to_room->y == (y - 1)) && (pexit->to_room->z == (z + 0)) ) { safe = TRUE; } break;
                     case 9: if ( (pexit->to_room->x == (x - 1)) && (pexit->to_room->y == (y - 1)) && (pexit->to_room->z == (z + 0)) ) { safe = TRUE; } break;
                  }
              ...
              {
                  default: break;
                  case 0:  map_apply_coords( pexit->to_room, x + 0, y + 1, z + 0 ); break;
                  case 1:  map_apply_coords( pexit->to_room, x + 1, y + 0, z + 0 ); break;
                  case 2:  map_apply_coords( pexit->to_room, x + 0, y - 1, z + 0 ); break;
                  case 3:  map_apply_coords( pexit->to_room, x - 1, y + 0, z + 0 ); break;
                  case 4:  map_apply_coords( pexit->to_room, x + 0, y + 0, z + 1 ); break;
                  case 5:  map_apply_coords( pexit->to_room, x + 0, y + 0, z - 1 ); break;
                  case 6:  map_apply_coords( pexit->to_room, x + 1, y + 1, z + 0 ); break;
                  case 7:  map_apply_coords( pexit->to_room, x - 1, y + 1, z + 0 ); break;
                  case 8:  map_apply_coords( pexit->to_room, x + 1, y - 1, z + 0 ); break;
                  case 9:  map_apply_coords( pexit->to_room, x - 1, y - 1, z + 0 ); break;
              }
          }
       }
    }    	                
...
ROOM_INDEX_DATA * room_from_coord( int x, int y, int z )
{
    ...
    for ( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
        for ( pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
	{
            if ( xIS_SET( pRoomIndex->room_flags, ROOM_NOCOORD ) ) continue;
...
void deploy_tns( int first, int last, int flag )
{
   ...
   for ( vnum = first; vnum < last; vnum++ )
   {
       ...
       if ( !xIS_SET( room->room_flags, flag ) ) continue;

       /* Generate a point - Adds to end of list */
       CREATE( tmp, TNS_DATA, 1 );
       tmp->prev = last_tns;
       if ( last_tns ) last_tns->next = tmp;
       if ( !first_tns ) first_tns = tmp;
       last_tns = tmp;
   ...
   for ( ; ; )
   {
       ...
       for ( tnsA = first_tns; tnsA; tnsA = tnsA->next )
       {
           ...
           if ( dist < ldist )
           {
               ...
               if ( tnsA->prev )
               { if ( tnsA->prev->prev ) tnsA->prev->prev->next = tnsA; }
               if ( tnsA->next ) tnsA->next->prev = tnsA->prev;
               tmpA = tnsA->next;
               tmpB = tnsA->prev->prev;
               tnsA->next = tnsA->prev;
               tnsA->prev->next = tmpA;
   ...
   for ( tnsA = first_tns; tnsA; tnsA = tnsA->next )
   {
       ...
       {
          if ( tnsB == tnsA ) continue;
          if ( !fpass && !tnsB->done ) continue;
       ...
       if ( tnsB != NULL )
       {
          ...
          for( ; stay ; )
          {
             if ( z < tnsB->z && !prev->link[4] ) { z++; dir = 4; }
             else if ( z > tnsB->z && !prev->link[5] ) { z--; dir = 5; }
             else if ( x < tnsB->x && !prev->link[1] ) { x++; dir = 1; }
             else if ( x > tnsB->x && !prev->link[3] ) { x--; dir = 3; }
             else if ( y < tnsB->y && !prev->link[0] ) { y++; dir = 0; }
             else if ( y > tnsB->y && !prev->link[2] ) { y--; dir = 2; }
             else { bug("TNS Fault!"); break; }
   ...
   for ( tnsA = first_tns; tnsA; tnsA = tnsA->next )
   {
       ...
       if ( room->name ) STRFREE( room->name );
       if ( room->description ) STRFREE( room->description );

       room->description = STRALLOC( desc );

       if ( room == NULL ) { cnt = -1; break; }
       ...
       if ( tnsA->main )
       {
          ...
          if ( room->link ) room->link->link = room;
   ...
   for ( tnsA = first_tns; tnsA; tnsA = tnsB )
   {
       ...
       for ( cnt = 0; cnt <= 5; cnt++ ) tnsA->link[cnt] = NULL;
  ...
  for ( vnum = sArea->low_r_vnum+1; vnum <= sArea->hi_r_vnum-1; vnum++ )
  {
     if ( (room = get_room_index(vnum)) != NULL )
     {
         ...
         for ( victim = room->first_person; victim; victim = vnext )
         {
            ...
            if ( IS_NPC(victim) && !xIS_SET(victim->act, ACT_POLYMORPHED) ) extract_char( victim, TRUE, FALSE );
         }
