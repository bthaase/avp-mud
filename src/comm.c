...
#endif
{
  ...
  /*  set_boot_time->hour   = 6;
    ...
    if ( wizlock ) log_string( "Wizlock automaticly engaged." );
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
...

	{
		...
		if ( pHelp->text[0] == '.' ) send_to_buffer( pHelp->text+1, d );
        ...
        if ( arg[0] == '\0' || arg2[0] == '\0' || strlen(arg) < 3 || strlen(arg2) < 3 )
	{
        ...
        for ( p = arg; *p != '\0'; p++ )
        { if ( *p == '~' ) { send_to_buffer( "&z(&CY&z)ou cannot include ~ in your name. Anywhere. Period.\n\r&zName: ", d ); return; } }

        for ( p = arg2; *p != '\0'; p++ )
        { if ( *p == '~' ) { send_to_buffer( "&z(&CY&z)ou cannot include ~ in your name. Anywhere. Period.\n\r&zName: ", d ); return; } }

        if ( ch->pcdata->fname ) DISPOSE( ch->pcdata->fname );
        if ( ch->pcdata->lname ) DISPOSE( ch->pcdata->lname );
	...
	send_to_buffer( buf, d );
        ...
        {
           if ( ch->race == RACE_ALIEN ) ch->sex = SEX_NEUTRAL;
           if ( ch->race == RACE_PREDATOR ) ch->sex = SEX_MALE;
           send_to_buffer( "\n\r&z(&CU&z)sing &YANSI&z Color by default. You may change this by using the CONFIG Command.\n\r", d );
           xSET_BIT( ch->act, PLR_ANSI );
           send_to_buffer( "\n\r&z(&CD&z)o you want to start with the language censor engaged? (&CY&z/&CN&z)?\n\r" , d );
           d->connected = CON_GET_CENSOR;
	...
	switch ( argument[0] )
	{
            case 'y': case 'Y': xSET_BIT(ch->act, PLR_CENSOR); break;
	    case 'n': case 'N': break;
	    default:
	...
	switch ( argument[0] )
	{
	    case 'y': case 'Y': xSET_BIT(ch->act,PLR_SOUND);  break;
	    case 'n': case 'N': break;
	    default:
    ...
    case CON_MENU_BASE:
        ...
        if ( !is_number( argument ) ) { c = -1; }        
        else { c = atoi( argument ); }
    ...
    case CON_MENU_SKILL1:
        ...
        if ( argument[0] == '\0' ) { c = 0; }
        else if ( !is_number( argument ) ) { c = -1; }
        else { c = atoi( argument ); }
        ...
        if ( ch->pcdata->learned[sn] <= 0 ) cost = 500;
        if ( ch->pcdata->learned[sn] == 1 ) cost = 1000;
        if ( ch->pcdata->learned[sn] == 2 ) cost = 2000;
    ...
    case CON_MENU_STAT1:
        ...
        if ( c == 1 ) sn = ch->perm_str;
        if ( c == 2 ) sn = ch->perm_sta;
        if ( c == 3 ) sn = ch->perm_rec;
        if ( c == 4 ) sn = ch->perm_per;
        if ( c == 5 ) sn = ch->perm_bra;
        if ( c == 6 ) sn = ch->perm_int;
    ...
    case CON_MENU_STAT2:
        ...
        if ( sn == 1 ) cost = ( ch->perm_str + 1 ) * 300;
        if ( sn == 2 ) cost = ( ch->perm_sta + 1 ) * 300;
        if ( sn == 3 ) cost = ( ch->perm_rec + 1 ) * 300;
        if ( sn == 4 ) cost = ( ch->perm_per + 1 ) * 300;
        if ( sn == 5 ) cost = ( ch->perm_bra + 1 ) * 300;
        if ( sn == 6 ) cost = ( ch->perm_int + 1 ) * 300;
...
bool check_parse_name( char *name )
{
    ...
    for ( bot = first_bot; bot; bot = bot->next )
      if ( !str_cmp( bot->name, name ) ) return FALSE;
...
bool check_reconnect( DESCRIPTOR_DATA *d, char *name, bool fConn )
{
	...
	&&   !str_cmp( name, ch->name ) )
	{
		...
		if ( !IS_IMMORTAL( ch ) ) act( AT_ACTION, "$n has reconnected.", ch, NULL, NULL, TO_ROOM );
...
void set_char_color( sh_int AType, CHAR_DATA *ch )
{
    ...
    if ( ch->desc ) och = (ch->desc->original ? ch->desc->original : ch);
    if ( !ch->desc ) return;
  ...
  while ( *str != '\0' )
  {
    ...
    else
    {
      ...
      case 'p': i = (!to || can_see_obj(to, obj1)
		  ? stripclr(obj_short(obj1)) : "something");           break;
      case 'P': i = (!to || can_see_obj(to, obj2)
		  ? stripclr(obj_short(obj2)) : "something");           break;
      case 'd':
...
void display_prompt( DESCRIPTOR_DATA *d )
{
  ...
  for ( ; *prompt; prompt++ )
  {
	...
	*pbuf = '\0';
	...
	if (percent >= 60) sprintf (pbuf, "%s%d%%%s", ANSISAFE(ansi, "\x1b[1;32m"), percent, ANSISAFE(ansi, "\x1b[0m") );
	else if (percent >= 30 && percent < 60) sprintf (pbuf, "%s%d%%%s", ANSISAFE(ansi, "\x1b[1;33m"), percent, ANSISAFE(ansi, "\x1b[0m") );
	else sprintf (pbuf, "%s%d%%%s", ANSISAFE(ansi, "\x1b[1;31m"), percent, ANSISAFE(ansi, "\x1b[0m") );
	...
	break;
      case 'K':          // Predator Vision Mode
        {
           ...
           if ( ch->vision == RACE_ALIEN ) sprintf( pbuf, "%s+++", ANSISAFE(ansi, "\x1b[1;34m") );
           if ( ch->vision == RACE_MARINE ) sprintf( pbuf, "%s+++", ANSISAFE(ansi, "\x1b[1;31m") );
           if ( ch->vision == RACE_PREDATOR ) sprintf( pbuf, "%s+++", ANSISAFE(ansi, "\x1b[1;32m") );
        }
        break;
      ...
      case 'p':          // Graphic AP prompt
        {
           ...
           if ( max > 0 ) percent = ( 100 * ch->ap ) / max;
 
           if ( ( max - ch->ap ) <= 0 ) sprintf(pbuf, "%s", ANSISAFE(ansi, "\x1b[1;32m"));
           else if ( ( max - ch->ap ) <= 2 ) sprintf(pbuf, "%s", ANSISAFE(ansi, "\x1b[1;33m"));
           else sprintf(pbuf, "%s", ANSISAFE(ansi, "\x1b[1;31m"));
        
           for ( ; max > 0; max--, cnt++ )
           {
               if ( cnt <= ch->ap ) strcat( pbuf, "|" );
               else strcat( pbuf, " " );
           }
        }
        break;
      case 'P':          // Graphic MP prompt
        {
           ...
           if ( max > 0 ) percent = ( 100 * ch->mp ) / max;
 
           if ( ( max - ch->mp ) <= 0 ) sprintf(pbuf, "%s", ANSISAFE(ansi, "\x1b[1;32m"));
           else if ( ( max - ch->mp ) <= 2 ) sprintf(pbuf, "%s", ANSISAFE(ansi, "\x1b[1;33m"));
           else sprintf(pbuf, "%s", ANSISAFE(ansi, "\x1b[1;31m"));
        
           for ( ; max > 0; max--, cnt++ )
           {
               if ( cnt <= ch->mp ) strcat( pbuf, "|" );
               else strcat( pbuf, " " );
           }
        }
        break;
      case 'A':                     // Percentage armor prompt
        ...
        */
        ...
        if (marmor <= 0) sprintf (pbuf, "%s%d%%%s", ANSISAFE(ansi, "\x1b[1;32m"), percent, ANSISAFE(ansi, "\x1b[0m") );
        else if (percent >= 60) sprintf (pbuf, "%s%d%%%s", ANSISAFE(ansi, "\x1b[1;32m"), percent, ANSISAFE(ansi, "\x1b[0m") );
        else if (percent >= 30 && percent < 60) sprintf (pbuf, "%s%d%%%s", ANSISAFE(ansi, "\x1b[1;33m"), percent, ANSISAFE(ansi, "\x1b[0m") );
        else sprintf (pbuf, "%s%d%%%s", ANSISAFE(ansi, "\x1b[1;31m"), percent, ANSISAFE(ansi, "\x1b[0m") );
	...
	else if ( time_info.hour < 20 ) strcpy( pbuf, "dusk" );
	...
	break;
      case 'J':
        ...
        if ( ch->in_room->area )
        {
             if ( ch->in_room->area->ambience == 1 ) strcpy( pbuf, "quiet" );
             if ( ch->in_room->area->ambience == 2 ) strcpy( pbuf, "average" );
             if ( ch->in_room->area->ambience == 3 ) strcpy( pbuf, "loud" );
        }
	break;
      case 'r':
	if ( IS_IMMORTAL(och) )
	...
	break;
      case 'b':          // OOC Limit-break
       if ( ch->pcdata->ooclimit > 6 ) ch->pcdata->ooclimit = 6;
       if ( ch->pcdata->ooclimit <= 6 ) sprintf( pbuf, "%s%d", ANSISAFE(ansi, "\x1b[1;32m"), ch->pcdata->ooclimit );
       if ( ch->pcdata->ooclimit <= 4 ) sprintf( pbuf, "%s%d", ANSISAFE(ansi, "\x1b[1;33m"), ch->pcdata->ooclimit );
       if ( ch->pcdata->ooclimit <= 2 ) sprintf( pbuf, "%s%d", ANSISAFE(ansi, "\x1b[1;31m"), ch->pcdata->ooclimit );
       if ( ch->pcdata->oocbreak ) sprintf( pbuf, "%sLB", ANSISAFE(ansi, "\x1b[1;31m") );
       break;                                         
      case 'B':          // OOC Limit-break
       if ( ch->pcdata->ooclimit > 6 ) ch->pcdata->ooclimit = 6;
       if ( ch->pcdata->ooclimit == 6 ) sprintf( pbuf, "%s||||||", ANSISAFE(ansi, "\x1b[1;32m") );
       if ( ch->pcdata->ooclimit == 5 ) sprintf( pbuf, "%s|||||-", ANSISAFE(ansi, "\x1b[1;32m") );
       if ( ch->pcdata->ooclimit == 4 ) sprintf( pbuf, "%s||||--", ANSISAFE(ansi, "\x1b[1;33m") );
       if ( ch->pcdata->ooclimit == 3 ) sprintf( pbuf, "%s|||---", ANSISAFE(ansi, "\x1b[1;33m") );
       if ( ch->pcdata->ooclimit == 2 ) sprintf( pbuf, "%s||----", ANSISAFE(ansi, "\x1b[1;31m") );
       if ( ch->pcdata->ooclimit == 1 ) sprintf( pbuf, "%s|-----", ANSISAFE(ansi, "\x1b[1;31m") );
       if ( ch->pcdata->ooclimit == 0 ) sprintf( pbuf, "%s------", ANSISAFE(ansi, "\x1b[1;31m") );
       if ( ch->pcdata->oocbreak ) sprintf( pbuf, "%s--LB--", ANSISAFE(ansi, "\x1b[1;31m") );
       break;                                         
    case 'I':
...
bool open_match( void )
{
   ...
   if ( ( fpMatch = fopen( filename, "w" ) ) == NULL ) return FALSE;
