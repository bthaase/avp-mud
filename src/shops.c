...
void do_buy( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_OBJ_STAT( obj, ITEM_INVENTORY ) ) separate_obj( obj );
    ...
    for ( obj = keeper->last_carrying; obj; obj = obj->prev_content )
    {
		...
		if ( !found )
		{
		    found = TRUE;
                ...
                if ( ch->pcdata )
                {
                   if ( obj->item_type == ITEM_MEDICOMP && ch->pcdata->learned[gsn_medical] < 3 ) warning = TRUE;
                   if ( obj->item_type == ITEM_SENTRY && ch->pcdata->learned[gsn_electronics] < 2 ) warning = TRUE;

                }
                if ( obj->item_type == ITEM_WEAPON && get_obj_weight( obj ) > get_curr_str(ch) ) warning = TRUE;
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !xIS_SET( obj->extra_flags, ITEM_RESPAWN ) ) cut = 25;

    if ( ch->race == RACE_MARINE )
    {
       if ( xIS_SET( obj->extra_flags, ITEM_ALIEN ) ) cut = 125;
       if ( xIS_SET( obj->extra_flags, ITEM_PREDATOR ) ) cut = 100;
    }
    else if ( ch->race == RACE_ALIEN )
    {
       if ( xIS_SET( obj->extra_flags, ITEM_MARINE ) ) cut = 50;
       if ( xIS_SET( obj->extra_flags, ITEM_PREDATOR ) ) cut = 75;
    }
    else if ( ch->race == RACE_PREDATOR )
    {
       if ( xIS_SET( obj->extra_flags, ITEM_ALIEN ) ) cut = 100;
       if ( xIS_SET( obj->extra_flags, ITEM_MARINE ) ) cut = 75;
    }
