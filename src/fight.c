...
void violence_update( void )
{
    ...
    for ( ch = last_char; ch; lst_ch = ch, ch = gch_prev )
    {
        ...
        */
        if ( xIS_SET( ch->in_room->room_flags, ROOM_HIVED ) )
            if ( ch->race == RACE_ALIEN )
                ch->resin = UMIN( get_max_resin( ch ), ch->resin + 1 );
        ...
        for ( paf = ch->first_affect; paf; paf = paf_next )
        {
            ...
            if ( paf->duration > 0 )
            {
                ...
                if ( paf->type == gsn_confuse && ch->in_room )
                {
                    ...
                    if ( !IS_NPC( ch ) )
                        range = 2 + ch->pcdata->learned[gsn_confuse];
        ...
        */
        if ( IS_BOT( ch ) || !IS_NPC( ch ) || ch->ap < get_max_ap( ch ) )
            continue;
        ...
        if ( rch == NULL )
        {
            ...
            */
            for ( rch = ch->in_room->first_person; rch; rch = rch_next )
            {
                ...
                if ( !can_see( ch, rch ) || ch->vent != rch->vent || is_spectator( rch ) )
                    continue;

                if ( is_enemy( ch, rch ) )
                {
                    if ( ch->race == RACE_MARINE )
                        do_emote( ch, "screams 'Bloody hell!'" );

                    if ( ch->race == RACE_ALIEN )
                        do_emote( ch, "screechs in anger!" );
...
ch_ret multi_hit( CHAR_DATA* ch, CHAR_DATA* victim, int dt )
{
    ...
    */
    if ( !IS_NPC( ch ) && ch->race == RACE_PREDATOR && !char_died( victim ) && !IS_NPC( victim ) )
    {
        ...
        if ( ch->pcdata->prepared[gsn_double_strike] < skill_table[gsn_double_strike]->reset )
            strike = 0;
        ...
        if ( !objA || is_ranged( objA->value[0] ) )
            objA = NULL;

        if ( !objB || is_ranged( objB->value[0] ) )
            objB = NULL;
...
ch_ret one_hit( CHAR_DATA* ch, CHAR_DATA* victim, int dt )
{
    ...
    */
    if ( dt == TYPE_UNDEFINED )
    {
        ...
        else
        {
            ...
            if ( ch->race != RACE_MARINE )
                dt = TYPE_GENERIC + RIS_PIERCE;

            if ( ch->race == RACE_MARINE )
                dt = TYPE_GENERIC + RIS_IMPACT;
        }
    }
    ...
    */
    ...
    if ( wield )
    {
        ...
        else if ( is_ranged( wield->value[0] ) )
        {
            ...
            if ( victim->pcdata && victim->hit > 0 )
            {
                ...
                if ( victim->pcdata->prepared[gsn_dodge] < skill_table[gsn_dodge]->reset )
                    dodge = 0;

                if ( victim->pcdata->prepared[gsn_primal_instinct] < skill_table[gsn_primal_instinct]->reset )
                    primal = 0;
        ...
        if ( wield->weapon_mode == MODE_SEMIAUTO )
            chance += 30;

        if ( wield->weapon_mode == MODE_AUTOMATIC )
            chance += 20;
    ...
    if ( chance > -100 )
        chance = stamina_penalty( ch, chance );

    /* Alien accuracy bonus */
    if ( ch->race == RACE_ALIEN && chance > -100 )
        chance += 20;

    /* Factor in character skills */
    if ( chance > -100 )
        chance = char_acc_modify( victim, chance );
    ...
    */
    if ( !wield )
    {
        ...
        switch ( ch->race )
        {
            ...
            case RACE_ALIEN:
                ...
                if ( ch->swarm > 0 )
                    dam += ( int )( ( float )( ( float )( dam ) / ( float )( 100 ) ) * ( float )( ch->swarm ) );
    ...
    else
    {
        ...
        if ( wield->ammo )
        {
            ...
            {
                if ( wield->value[3] > wield->ammo->value[2] )
                    rounds = wield->ammo->value[2];
                else
                    rounds = wield->value[3];
            ...
            {
                if ( wield->value[4] > wield->ammo->value[2] )
                    rounds = wield->ammo->value[2];
                else
                    rounds = wield->value[4];
    ...
    if ( diceroll == 1 || diceroll > chance )
    {
        ...
        if ( wield )
        {
            ...
            {
                if ( wield->weapon_mode == MODE_AUTOMATIC )
                    miss_message( ch, victim, -2, wield->ammo->value[0] );
                else if ( wield->weapon_mode == MODE_BURST )
                    miss_message( ch, victim, -1, wield->ammo->value[0] );
                else
                    miss_message( ch, victim, wield->value[0], wield->ammo->value[0] );
            }
            else if ( wield->value[0] == WEAPON_ERANGED )
            {
                if ( wield->weapon_mode == MODE_AUTOMATIC )
                    miss_message( ch, victim, -2, 1 );
                else if ( wield->weapon_mode == MODE_BURST )
                    miss_message( ch, victim, -1, 1 );
                else
                    miss_message( ch, victim, wield->value[0], 1 );
            }
            else
            {
    ...
    */
    if ( retcode == rNONE && flame )
    {
        ...
        if ( victim->race == RACE_ALIEN )
            duration = URANGE( 3, get_trust( victim ), 8 );

        if ( victim->race == RACE_MARINE )
            duration = URANGE( 4, get_trust( victim ), 12 );

        if ( victim->race == RACE_PREDATOR )
            duration = URANGE( 5, get_trust( victim ), 12 );
    ...
    */
    if ( retcode == rNONE && !flame && victim->race == RACE_ALIEN && ch->in_room == victim->in_room && number_percent( ) > ( ( get_curr_per( ch ) * 2.4 ) + 10 ) )
    {
        ...
        if ( victim->pcdata )
        {
            if ( victim->pcdata->learned[gsn_acid_potency] == 1 )
                spray *= 1.10;

            if ( victim->pcdata->learned[gsn_acid_potency] == 2 )
                spray *= 1.20;

            if ( victim->pcdata->learned[gsn_acid_potency] == 3 )
                spray *= 1.30;
        }
    ...
    if ( apply )
    {
        for ( obj = ch->first_carrying; obj; obj = obj_next )
        {
            ...
            for ( paf = obj->pIndexData->first_affect; paf; paf = paf->next )
            {
                ...
                if ( paf->location == APPLY_FIRE && ris == RIS_FIRE )
                    value = paf->modifier;

                if ( paf->location == APPLY_ENERGY && ris == RIS_ENERGY )
                    value = paf->modifier;

                if ( paf->location == APPLY_IMPACT && ris == RIS_IMPACT )
                    value = paf->modifier;

                if ( paf->location == APPLY_PIERCE && ris == RIS_PIERCE )
                    value = paf->modifier;

                if ( paf->location == APPLY_ACID && ris == RIS_ACID )
                    value = paf->modifier;
                ...
                if ( value != 0 )
                {
                    ...
                    if ( damage_obj( obj, odam ) == rOBJ_SCRAPPED )
                        break;
                }
            }
...
ch_ret damage( CHAR_DATA* ch, CHAR_DATA* victim, int dam, int dt )
{
    ...
    */
    if ( victim->pcdata && victim->hit > 0 && dam > 0 )
    {
        ...
        if ( victim->pcdata->prepared[gsn_near_miss] < skill_table[gsn_near_miss]->reset )
            near = 0;
    ...
    */
    for ( paf = victim->first_affect; paf; paf = paf->next )
        if ( paf->type == gsn_alien_rage && victim->in_room )
            dam /= 2;
    ...
    if ( victim->position == POS_DEAD )
    {
        ...
        if ( !npcvict && ch != victim )
        {
            ...
            for ( d = first_descriptor; d; d = d->next )
            {
                ...
                if ( msg == 1 )
                    ch_printf( d->character, "&R(Alert) %s bashed %s with a whuppin' stick! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 2 )
                    ch_printf( d->character, "&R(Alert) %s pasted %s! Ouch! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 3 )
                    ch_printf( d->character, "&R(Alert) %s learned %s some manners! Bwhahaha! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 4 )
                    ch_printf( d->character, "&R(Alert) %s opened a can of whoop-ass on %s! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 5 )
                    ch_printf( d->character, "&R(Alert) %s blasted %s's ass off! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 6 )
                    ch_printf( d->character, "&R(Alert) %s showed %s where the trigger was! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 7 )
                    ch_printf( d->character, "&R(Alert) %s taught %s where not to stand! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 8 )
                    ch_printf( d->character, "&R(Alert) %s thought %s needed to pay more attention! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 9 )
                    ch_printf( d->character, "&R(Alert) %s told %s where he could shove it! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 10 )
                    ch_printf( d->character, "&R(Alert) %s rearranged %s's facial features! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 11 )
                    ch_printf( d->character, "&R(Alert) %s demonstrated the art of ass-kicking for %s! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );

                if ( msg == 12 )
                    ch_printf( d->character, "&R(Alert) %s bitchslapped %s into submission! %s\n\r", ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, note );
            }
        }
...
void check_killer( CHAR_DATA* ch, CHAR_DATA* victim )
{
    if ( ch == victim )
    {
        ...
        {
            if ( IS_NPC( ch ) )
                match_log( "KILL;'NPC:%s' suicided with '%s'", ch->name, current_weapon( ch ) );

            if ( !IS_NPC( ch ) )
                match_log( "KILL;'PC:%s' suicided with '%s'", ch->name, current_weapon( ch ) );
        }
    ...
    else if ( IS_BOT( ch ) )
    {
        ...
        {
            if ( !IS_BOT( victim ) )
                match_log( "KILL;'BOT:%s' killed 'NPC:%s' with '%s'", ch->name, victim->name, current_weapon( ch ) );

            if ( IS_BOT( victim ) )
                match_log( "KILL;'BOT:%s' killed 'BOT:%s' with '%s'", ch->name, victim->name, current_weapon( ch ) );
        ...
        else
        {
            ...
            if ( victim->pcdata )
                victim->pcdata->killed.bot[ch->race]++;
        }
    }
    else if ( IS_NPC( ch ) )
    {
        ...
        {
            if ( !IS_BOT( victim ) )
                match_log( "KILL;'NPC:%s' killed 'NPC:%s' with '%s'", ch->name, victim->name, current_weapon( ch ) );

            if ( IS_BOT( victim ) )
                match_log( "KILL;'NPC:%s' killed 'BOT:%s' with '%s'", ch->name, victim->name, current_weapon( ch ) );
        ...
        else
        {
            ...
            if ( victim->pcdata )
                victim->pcdata->killed.npc[ch->race]++;
        }
    }
    else
    {
        if ( IS_BOT( victim ) )
        {
            ...
            if ( ch->pcdata )
                ch->pcdata->kills.bot[victim->race]++;
        }
        else if ( IS_NPC( victim ) )
        {
            ...
            if ( ch->pcdata )
                ch->pcdata->kills.npc[victim->race]++;
        }
        else
        {
            ...
            if ( curr_arena )
                curr_arena->kills[ch->race]++;

            if ( ch->pcdata )
                ch->pcdata->kills.pc[victim->race]++;

            if ( victim->pcdata )
                victim->pcdata->killed.pc[ch->race]++;
        }
    }
...
void raw_kill( CHAR_DATA* ch, CHAR_DATA* victim )
{
    ...
    if ( !IS_NPC( victim ) )
        match_log( "DEATH;'PC:%s' killed while using '%s'", victim->name, current_weapon( victim ) );
...
void do_tackle( CHAR_DATA* ch, char* argument )
{
    ...
    */
    ...
    if ( objB == NULL )
        objB = get_eq_char( victim, WEAR_HOLD );

    if ( objA != NULL )
        if ( xIS_SET( objA->extra_flags, ITEM_NOTACKLE ) )
            objA = NULL;

    if ( objB != NULL )
        if ( xIS_SET( objB->extra_flags, ITEM_NOTACKLE ) )
            objB = NULL;

    if ( objA != NULL )
        chance += ( objA->weight * 2 );

    if ( objB != NULL )
        chance += ( objB->weight * 2 );
...
void radius_gain( CHAR_DATA* ch, CHAR_DATA* victim )
{
    ...
    */
    ...
    if ( ch->race == RACE_PREDATOR )
    {
        ...
        if ( IS_AFFECTED( ch, AFF_CLOAK ) && victim->race != RACE_ALIEN )
            exp /= 4;
    }
...
void xp_radius_2( ROOM_INDEX_DATA* room, int range )
{
    ...
    if ( range > 0 )
    {
        ...
        {
            if ( pexit->to_room && pexit->to_room != room )
                xp_radius_2( pexit->to_room, range - 1 );
        }
    }
...
void res_radius_2( ROOM_INDEX_DATA* room, int range )
{
    ...
    if ( range > 0 )
    {
        ...
        {
            if ( pexit->to_room && pexit->to_room != room )
                res_radius_2( pexit->to_room, range - 1 );
        }
    }
...
int apply_armor_skill( CHAR_DATA* ch, OBJ_DATA* obj, int odam )
{
    ...
    if ( obj->item_type == ITEM_ARMOR )
    {
        ...
        {
            case 1:
                pr = ch->pcdata->learned[gsn_lightarmor];
                break;

            case 2:
                pr = ch->pcdata->learned[gsn_mediumarmor];
                break;

            case 3:
                pr = ch->pcdata->learned[gsn_heavyarmor];
                break;
...
int default_weapon_mode( OBJ_DATA* obj )
{
    ...
    if ( xIS_SET( obj->extra_flags, ITEM_SINGLEFIRE ) )
        return MODE_SINGLE;

    if ( xIS_SET( obj->extra_flags, ITEM_SEMIAUTO ) )
        return MODE_SEMIAUTO;
...
void do_spray( CHAR_DATA* ch, char* argument )
{
    ...
    // Execute Spray
    if ( ret >= 0 )
        ret += spray_direction( ch, wieldA, dir, 1 );

    if ( ret >= 0 )
        ret += spray_direction( ch, wieldB, dir, 2 );
...
int spray_direction( CHAR_DATA* ch, OBJ_DATA* wield, int dir, int dual )
{
    ...
    {
        if ( dual == 1 )
            send_to_char( "&RYou dont seem to have a weapon!\n\r", ch );
    ...
    {
        if ( dual == 1 )
            send_to_char( "&RThats... not a weapon.\n\r", ch );
    ...
    {
        if ( dual == 1 )
            send_to_char( "&RUse RANGED Weapons. RANGED.\n\r", ch );
    ...
    {
        if ( dual == 1 )
            send_to_char( "&RLoad the gun first. Trust me on this.\n\r", ch );
    ...
    {
        if ( dual == 1 )
            send_to_char( "&RTry picking a gun with range, buddy.\n\r", ch );
    ...
    {
        if ( dual == 1 )
            send_to_char( "&RUse SETMODE to select either BURST or AUTOMATIC mode first.\n\r", ch );
    ...
    for ( range = 1; range <= mrange; range++ )
    {
        ...
        for ( rch = to_room->first_person; rch; rch = rnext )
        {
            ...
            if ( !flame )
            {
                ...
                if ( IS_OBJ_STAT( wield, ITEM_FULLHIT ) )
                    chance = number_range( ( int )( ( float )( rounds ) / ( float )( 2 ) ), rounds );

                chance = URANGE( -1, chance, rounds );

                if ( !can_see( ch, rch ) && !IS_OBJ_STAT( wield, ITEM_FULLHIT ) )
                    chance = ( int )( ( float )( rounds ) / ( float )( 2 ) );

                if ( rch->position <= POS_PRONE )
                    chance = 0;
            }
            /* Flamethrowers */
            else
            ...
            if ( rch->pcdata && rch->hit > 0 )
            {
                ...
                if ( rch->pcdata->prepared[gsn_dodge] < skill_table[gsn_dodge]->reset )
                    dodge = 0;

                if ( rch->pcdata->prepared[gsn_primal_instinct] < skill_table[gsn_primal_instinct]->reset )
                    primal = 0;
            ...
            if ( chance > 0 )
            {
                ...
                if ( ( dam = cdamage( ch, rch, dam, TRUE ) ) > -1 )
                {
                    ...
                    if ( flame )
                        ignite_target( ch, rch, number_range( 5, 10 ) );
                }
...
int snipe_direction( CHAR_DATA* ch, CHAR_DATA* vic, char* arg, OBJ_DATA* wield, int dir, int dual )
{
    ...
    {
        if ( dual == 1 )
            send_to_char( "&RNot enough field charge to fire this weapon.\n\r", ch );
    ...
    for ( range = 1; range <= mrange; range++ )
    {
        ...
        {
            if ( ( rch = get_char_far_room( ch, to_room, arg ) ) != NULL )
                found = TRUE;
        }
...
int cdamage( CHAR_DATA* ch, CHAR_DATA* victim, int dam, bool silent )
{
    ...
    {
        if ( !silent )
            ch_printf( ch, "&YYour attack strikes a %s! (%d damage)\n\r", victim->cover->short_descr, dam );
    ...
    else
    {
        if ( ( victim->cover->value[3] - ndam ) <= 0 )
        {
            ...
            if ( !silent )
                ch_printf( ch, "&RYour attack devastates a %s! (%d damage)\n\r", victim->cover->short_descr, ndam );
...
bool is_enemy( CHAR_DATA* ch, CHAR_DATA* victim )
{
    if ( ( IS_NPC( ch ) && xIS_SET( ch->act, ACT_HOSTAGE ) ) && victim->race == RACE_MARINE )
        return FALSE;

    if ( ( IS_NPC( ch ) && xIS_SET( victim->act, ACT_HOSTAGE ) ) && ch->race == RACE_MARINE )
        return FALSE;
    ...
    if ( IS_NPC( ch ) && IS_NPC( victim ) )
    {
        if ( xIS_SET( ch->act, ACT_AGGRESSIVE ) && xIS_SET( victim->act, ACT_AGGRESSIVE ) )
            return FALSE;
    }

    if ( IS_NPC( ch ) && xIS_SET( ch->act, ACT_AGGRESSIVE ) )
        return TRUE;

    if ( IS_NPC( victim ) && xIS_SET( victim->act, ACT_AGGRESSIVE ) )
        return TRUE;
...
void weapon_echo( CHAR_DATA* ch, OBJ_DATA* obj )
{
    ...
    if ( xIS_SET( obj->extra_flags, ITEM_LOUD ) )
        base = 6;

    if ( xIS_SET( obj->extra_flags, ITEM_MODERATE ) )
        base = 4;

    if ( xIS_SET( obj->extra_flags, ITEM_QUIET ) )
        base = 2;
...
void do_rescue( CHAR_DATA* ch, char* argument )
{
    ...
    if ( is_spectator( ch ) )
    {
        send_to_char( "&RYou may not rescue while in spectator mode.\n\r", ch );
        return;
    }
