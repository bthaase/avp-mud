...
void do_cyber ( CHAR_DATA* ch, char* argument )
{
    ...
    else if ( !str_cmp( arg, "muscle" ) )
    {
        ...
        if ( !c_afford( ch, mob, cost ) )
            return;

        if ( c_gotpart( ch, mob, CYBER_MUSCLE ) )
            return;
    ...
    else if ( !str_cmp( arg, "vision" ) )
    {
        ...
        if ( c_gotpart( ch, mob, CYBER_VISION ) )
            return;
    ...
    else if ( !str_cmp( arg, "bioplug" ) )
    {
        ...
        if ( c_gotpart( ch, mob, CYBER_BIOPLUG ) )
            return;
    ...
    else if ( !str_cmp( arg, "recover" ) )
    {
        ...
        if ( c_gotpart( ch, mob, CYBER_RECOVER ) )
            return;
    ...
    else if ( !str_cmp( arg, "oxygen" ) )
    {
        ...
        if ( c_gotpart( ch, mob, CYBER_OXYGEN ) )
            return;
    ...
    else if ( !str_cmp( arg, "immune" ) )
    {
        ...
        */
        ...
        if ( c_gotpart( ch, mob, CYBER_IMMUNE ) )
            return;
    ...
    if ( ch->move >= 50 && xIS_SET( ch->cyber_flags, CYBER_BIOPLUG ) )
    {
        ...
        if ( wield != NULL )
            if ( wield->item_type == ITEM_WEAPON && wield->value[4] < wield->value[5] )
                ...
                {
                    if ( wield->value[4] + 1 < wield->value[5] )
                        wield->value[4] += 1;
