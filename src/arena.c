...
void update_arena( void )
{
    ...
    if ( curr_arena )
    {
        ...
        if ( curr_arena->ctimer == 5 )
            send_monitor( NULL, "&YArena will evacuate in 5 minutes." );

        if ( curr_arena->ctimer == 1 )
            send_monitor( NULL, "&YArena will evacuate in 1 minute." );

        if ( curr_arena->ctimer > 0 )
            return;
    }
    ...
    for ( safty = 0; safty < 99999; safty++ )
    {
        ...
        if ( arenaNext > last_arena->id )
            arenaNext = first_arena->id;
        ...
        */
        ...
        if ( curr_arena )
        {
            if ( curr_arena->id == arena->id && last_arena->id > first_arena->id )
                continue;
        }

        if ( pcount < arena->min_p || pcount > arena->max_p )
            continue;

        if ( curr_arena )
            empty_arena( curr_arena );
...
void do_deploy( CHAR_DATA* ch, char* argument )
{
    ...
    if ( room != NULL )
    {
        ...
        if ( room->area )
            motion_ping( room->x, room->y, room->z, room->area, ch );
    }
    else
    {
        do_deploy( ch, "" );
