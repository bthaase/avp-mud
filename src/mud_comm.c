...
char *mprog_type_to_name( int type )
{
	...
	case DAMAGE_PROG:       return "damage_prog";
        case USEON_PROG:        return "useon_prog";
        case USEOFF_PROG:       return "useoff_prog";
	...
	case USE_PROG:          return "use_prog";
        case PULSE_PROG:        return "pulse_prog";
...
void do_mprmflag( CHAR_DATA * ch, char * argument )
{
    ...
    while ( argument[0] != '\0' )
    {
       ...
       else
       {
           ...
           {
              if ( state == 0 ) xSET_BIT( room->room_flags, value );
              if ( state == 1 ) xREMOVE_BIT( room->room_flags, value );
              if ( state == 2 ) xTOGGLE_BIT( room->room_flags, value );
           }
       }
    }
    return;
...
void do_mpblast( CHAR_DATA *ch, char *argument )
{
    ...
    if ( fr < ch->in_room->area->low_r_vnum ) fr = ch->in_room->area->low_r_vnum;
    if ( lr > ch->in_room->area->hi_r_vnum ) lr = ch->in_room->area->hi_r_vnum;
...
{ 
    ...
    if ( !str_cmp( arg1, "alien" ) ) { race = RACE_ALIEN; }
    else if ( !str_cmp( arg1, "marine" ) ) { race = RACE_MARINE; }
    else if ( !str_cmp( arg1, "predator" ) ) { race = RACE_PREDATOR; }
    else { race = atoi( arg1 ); }
