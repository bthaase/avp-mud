...
void do_drop( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IN_VENT( ch ) ) { send_to_char( "&RNot while inside a vent. Sorry.\n\r", ch ); return; }
...
obj_ret damage_obj( OBJ_DATA *obj, int dam )
{
    ...
    if ( obj->item_type == ITEM_SCRAPS ) return objcode;
    if ( obj->item_type == ITEM_CORPSE_NPC || obj->item_type == ITEM_CORPSE_PC ) return objcode;

    if ( IS_OBJ_STAT(obj, ITEM_INDESTRUCTABLE) ) return objcode;
	...
	default:
	  ...
	  break;
        case ITEM_COVER:
          ...
          if ( obj->value[3] <= 0 )
          {
             for ( ch = last_char; ch; ch = ch->prev ) { remove_cover( ch, obj ); }
             ...
             if ( ( obj->value[5] == 1 || obj->value[5] == 4 ) && room )
             {
                 for ( rch = room->first_person; rch; rch = rnext )
                 {
                      ...
                      if ( IN_VENT(rch) || is_spectator( rch ) ) continue;
                      if ( !IS_NPC( rch ) )
                      {
                      ...
                      if ( obj->value[5] == 1 ) ignite_target( rch, rch, obj->weight );
                 }
             }
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit )
{
    ...
    if ( obj->item_type == ITEM_MEDICOMP ) obj->value[0] = 0;
...
void do_junk( CHAR_DATA *ch, char *argument )
{
	...
	for ( obj = ch->in_room->first_content; obj; obj = obj_next )
	{
	    obj_next = obj->next_content;
            ...
            if ( obj->item_type == ITEM_GRENADE && obj->value[4] > 0 ) continue;
            if ( obj->item_type == ITEM_CORPSE_NPC || obj->item_type == ITEM_CORPSE_PC ) continue;
            if ( contains_explosive( obj, 0 ) ) continue;
            if ( obj->item_type == ITEM_SCRAPS ) continue;
            ...
            if ( cur_obj == obj->serial ) global_objcode = rOBJ_SACCED;
            extract_obj( obj );
	}
    ...
    /* SAVE_EQ room checks */
    if ( if_equip_room(ch->in_room) ) save_equip_room(ch, ch->in_room);
...
void detonate_traps( CHAR_DATA * ch, ROOM_INDEX_DATA * room )
{
    ...
    {        
       ...
       {
          if ( (ch->race+1) == robj->value[3] ) continue;
          if ( (ch->race+1) != robj->value[2] && robj->value[2] > 0 ) continue;

          if ( IS_OBJ_STAT( robj, ITEM_BURRIED ) ) explode( robj );
       }
       else if ( robj->item_type == ITEM_TRAP )
       {
          if ( !IS_OBJ_STAT( robj, ITEM_ALIEN ) && ch->race == RACE_ALIEN ) continue;
          if ( !IS_OBJ_STAT( robj, ITEM_MARINE ) && ch->race == RACE_MARINE ) continue;
          if ( !IS_OBJ_STAT( robj, ITEM_PREDATOR ) && ch->race == RACE_PREDATOR ) continue;
          ...
          if ( robj->value[0] == 7 ) // Damage
          {
             ...
             if ( robj->parent )
             {
               if ( !IS_NPC( robj->parent ) ) robj->parent->pcdata->prepared[gsn_battle_method]--;
          ...
          else if ( robj->value[0] == 5 ) // Ambush Traps
          {
             ...
             {
                if ( get_mob_index( robj->value[1] ) == NULL ) continue;
          ...
          if ( robj->value[5] > 0 )
          {
             if ( --robj->value[5] == 0 ) extract_obj( robj );
          }
       }

       if ( is_spectator( ch ) ) break;
    }
...
int count_traps( ROOM_INDEX_DATA * room )
{
    ...
    {        
       if ( robj->item_type == ITEM_LANDMINE )
       {
          if ( IS_OBJ_STAT( robj, ITEM_BURRIED ) ) cnt++;
       }
       else if ( robj->item_type == ITEM_TRAP )
       {
          cnt++;
...
void clear_traps( ROOM_INDEX_DATA * room )
{
    ...
    for ( robj = room->first_content; robj; robj = robj_next )
    {
       ...
       if ( robj->item_type == ITEM_LANDMINE )
       {            
          if ( IS_OBJ_STAT( robj, ITEM_BURRIED ) ) extract_obj( robj );
       }
       else if ( robj->item_type == ITEM_TRAP )
       {
          extract_obj( robj );
...
void equip_lag( CHAR_DATA * ch, OBJ_DATA * obj )
{
    ...
    if ( obj->wear_loc == WEAR_SHOULDER ) lag = 3;
