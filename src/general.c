...
void do_research( CHAR_DATA *ch, char *argument )
{
    ...
    { 
    	default:
          ...
          if ( is_lang )  adept = 95;
          if ( is_skill ) adept = URANGE( 20, (get_curr_int(ch)*5), 80 );
    ...
    if ( is_lang )  adept = 95;
    if ( is_skill ) adept = (get_curr_int(ch)*5);
    ...
    if ( is_lang && boost > 10 ) boost = 10;
    if ( !is_lang && boost > 15 ) boost = 15;
    if ( boost < 4 ) boost = 4;

    ch->pcdata->learned[sn] += boost;
    ch->pcdata->learned[sn] = URANGE( 0, ch->pcdata->learned[sn], 100 );
    if ( !is_lang ) ch->pcdata->learned[sn] = URANGE( 0, ch->pcdata->learned[sn], adept );
