...
void do_attach( CHAR_DATA *ch, char *argument )
{
    ...
    /* Flat Modifiers */
    if ( obj->value[0] == 2 ) tobj->value[2] += obj->value[4];
...
void do_detach( CHAR_DATA *ch, char *argument )
{
    ...
    /* Flat Modifiers */
    if ( obj->attach->value[0] == 2 ) obj->value[2] -= obj->attach->value[4];
...
void send_attach_stats( CHAR_DATA * ch, OBJ_DATA * obj )
{
    ...
    if ( obj->value[0] == 0 ) strcpy( buf, "Tactical Light" );
    if ( obj->value[0] == 1 ) strcpy( buf, "Grenade Launcher" );
    if ( obj->value[0] == 2 ) strcpy( buf, "Accuracy Modifer" );
    if ( obj->value[0] == 3 ) strcpy( buf, "Weapon Silencer" );
    if ( obj->value[0] == 4 ) strcpy( buf, "Damage Modifier" );
    if ( obj->value[0] == 5 ) strcpy( buf, "Motion Tracker" );
    if ( obj->value[0] == 6 ) strcpy( buf, "Vision Augment" );
    if ( obj->value[0] == 7 ) strcpy( buf, "Ammo Modifier" );
    if ( obj->value[0] == 8 ) strcpy( buf, "Fire Rate Modifier" );
    if ( obj->value[0] == 9 ) strcpy( buf, "Speed Loader" );
...
int attach_light_modifier( CHAR_DATA * ch )
{
   ...
   for ( obj = ch->first_carrying; obj; obj = obj->next_content )
   {
       ...
       * Flare patch
       */
      if ( obj->item_type == ITEM_FLARE && obj->value[2] != 0 ) mod++; 
...
int check_light_modifier( OBJ_DATA * obj )
{
   ...
   if ( ( tmp = obj->attach ) != NULL )
   {
      ...
      {
         if ( tmp->value[2] > 0 && tmp->value[4] == 1 ) return 2;
      }
   }
...
bool use_attachment( CHAR_DATA * ch, OBJ_DATA * tmp )
{
   ...
   if ( ( obj = tmp->attach ) == NULL ) return FALSE;
...
void recharge_attachment( CHAR_DATA * ch, OBJ_DATA * tmp )
{
   ...
   {
      case 0: // Attachable Lights
        if ( obj->value[4] == 0 ) obj->value[2] = UMIN( obj->value[3], obj->value[2] + 1 );
        if ( obj->value[4] == 1 )
        {
...
int fire_attachment( CHAR_DATA * ch, OBJ_DATA * obj, int dir, int range )
{
    ...
    if ( obj->value[0] != 1 ) return 0;

    to_room = ch->in_room;

    if ( range < 0 ) range = obj->value[3];
    if ( range > obj->value[3] ) range = obj->value[3];
...
bool has_thermal( CHAR_DATA * ch )
{
   ...
   for ( obj = ch->first_carrying; obj; obj = obj->next_content )
   {
      ...
      {
          if ( obj->attach->value[3] != 0 ) thermal = TRUE;
      }
   }
